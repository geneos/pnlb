 ...
 *	
 *  @author Jorg Janke
 *  @version $Id: AcctSchemaCopyAcct.java,v 1.2 2005/11/01 16:36:33 jjanke Exp $
 */
public class AcctSchemaCopyAcct extends SvrProcess
{
	...
	protected String doIt () throws Exception
	{
		...
		if (p_SourceAcctSchema_ID == 0 || p_TargetAcctSchema_ID == 0)
			throw new CompiereSystemError("ID=0");
		
		if (p_SourceAcctSchema_ID == p_TargetAcctSchema_ID)
			throw new CompiereUserError("Must be different");
		
		MAcctSchema source = MAcctSchema.get(getCtx(), p_SourceAcctSchema_ID, null);
		if (source.get_ID() == 0)
			throw new CompiereSystemError("NotFound Source C_AcctSchema_ID=" + p_SourceAcctSchema_ID);
		MAcctSchema target = new MAcctSchema (getCtx(), p_TargetAcctSchema_ID, get_TrxName());
		if (source.get_ID() == 0)
			throw new CompiereSystemError("NotFound Target C_AcctSchema_ID=" + p_TargetAcctSchema_ID);
		...
		if (targetElements.length == 0)
			throw new CompiereUserError("NotFound Target C_AcctSchema_Element");
		...
		if (sourceAcctElement == null)
			throw new CompiereUserError("NotFound Source AC C_AcctSchema_Element");
		...
		if (targetAcctElement == null)
			throw new CompiereUserError("NotFound Target AC C_AcctSchema_Element");
		if (sourceAcctElement.getC_Element_ID() != targetAcctElement.getC_Element_ID())
			throw new CompiereUserError("@C_Element_ID@ different");
	...
	private void copyGL (MAcctSchema targetAS) throws Exception
	{
		...
		if (!target.save())
			throw new CompiereSystemError("Could not Save GL");
	}	//	copyGL
	...
	private void copyDefault(MAcctSchema targetAS) throws Exception
	{
		...
		if (!target.save())
			throw new CompiereSystemError("Could not Save Default");
	}	//	copyDefault
