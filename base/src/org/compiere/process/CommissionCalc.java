 ...
 *	
 *  @author Jorg Janke
 *  @version $Id: CommissionCalc.java,v 1.17 2005/10/26 00:37:42 jjanke Exp $
 */
public class CommissionCalc extends SvrProcess
{
	...
	protected String doIt() throws Exception
	{
		...
		if (m_com.get_ID() == 0)
			throw new CompiereUserError ("No Commission");
		...
		if (!comRun.save())
			throw new CompiereSystemError ("Could not save Commission Run");
		...
		for (int i = 0; i < lines.length; i++)
		{
			...
			if (!comAmt.save())
				throw new CompiereSystemError ("Could not save Commission Amt");
			//
			...
			if (MCommission.DOCBASISTYPE_Receipt.equals(m_com.getDocBasisType()))
			{
				...
				if (m_com.isListDetails())
				{
					sql.append("SELECT h.C_Currency_ID, l.LineNetAmt, l.QtyInvoiced, "
						+ "NULL, l.C_InvoiceLine_ID, p.DocumentNo||'_'||h.DocumentNo, COALESCE(prd.Value,l.Description), h.DateInvoiced "
						+ "FROM C_Payment p"
						+ " INNER JOIN C_Invoice h ON (p.C_Invoice_ID = h.C_Invoice_ID)"
				...
				{
					sql.append("SELECT h.C_Currency_ID, SUM(l.LineNetAmt) AS Amt, SUM(l.QtyInvoiced) AS Qty, "
			...
			else if (MCommission.DOCBASISTYPE_Order.equals(m_com.getDocBasisType()))
			{
				if (m_com.isListDetails())
				{
					sql.append("SELECT h.C_Currency_ID, l.LineNetAmt, l.QtyOrdered, "
						+ "l.C_OrderLine_ID, NULL, h.DocumentNo, COALESCE(prd.Value,l.Description),h.DateOrdered "
						+ "FROM C_Order h"
				...
				{
					sql.append("SELECT h.C_Currency_ID, SUM(l.LineNetAmt) AS Amt, SUM(l.QtyOrdered) AS Qty, "
			...
			else 	//	Invoice Basis
			{
				...
				{
					sql.append("SELECT h.C_Currency_ID, l.LineNetAmt, l.QtyInvoiced, "
						+ "NULL, l.C_InvoiceLine_ID, h.DocumentNo, COALESCE(prd.Value,l.Description),h.DateInvoiced "
						+ "FROM C_Invoice h"
						+ " INNER JOIN C_InvoiceLine l ON (h.C_Invoice_ID = l.C_Invoice_ID)"
						+ " LEFT OUTER JOIN M_Product prd ON (l.M_Product_ID = prd.M_Product_ID) "
			...
			if (lines[i].isCommissionOrders())
			{
				...
				if (users == null || users.length == 0)
					throw new CompiereUserError ("Commission Business Partner has no Users/Contact");
				if (users.length == 1)
				{
			...
			if (lines[i].getC_BP_Group_ID() != 0)
				sql.append(" AND h.C_BPartner_ID IN "
					+ "(SELECT C_BPartner_ID FROM C_BPartner WHERE C_BP_Group_ID=").append(lines[i].getC_BP_Group_ID()).append(")");
                        //begin vpj-cd 11 May 2006
                        if (lines[i].getC_BP_Group_ID() != 0)
				sql.append(" AND h.C_BPartner_Location_ID = ").append(lines[i].getC_BPartner_Location_ID());    
                        //end vpj-cd 11 May 2006
			//	Sales Region
