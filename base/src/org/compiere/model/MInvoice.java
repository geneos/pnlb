...
public class MInvoice extends X_C_Invoice implements DocAction {
    ...
    public String toString() {
        StringBuffer sb = new StringBuffer("MInvoice[").append(get_ID()).append("-").append(getDocumentNo()).append(",GrandTotal=").append(getGrandTotal());
        if (m_lines != null) {
    ...
    public String completeIt() {
        ...
        } else // GENEOS - Pablo Velazquez: Toma cotizacion segun fecha de contabilidad
        {
    ...
    public boolean anularComprobantesAsociados() {
        ...
        for (int i = 0; i < mInv.length; i++) {	//	Anular remito de cada linea asociada, generalmente es s�lo uno.
...
/*		for (int i = 0; i < getLines().length; i++)
    ...
    public boolean voidIt() {
        ...
        if (DOCSTATUS_Drafted.equals(getDocStatus())
            ...
            if (verificarAnular()) {
                if (isSOTrx()) {
                    if (getC_Order_ID() != 0) //�	Factura, Remito y Nota de Cr�dito deben ser anulados y se desasocia la Orden de venta.
                    {
                    ...
                    } else //�	Si la Factura fue creada desde un Remito el mismo debe ser desasociado, no anulado. Y ser� posible volver a crear una Factura a partir de �l mediante Crear desde.
                    {
                ...
                } else if (docType.getDocBaseType().equals("ARC") || docType.getDocBaseType().equals("API")) {
    ...
    public boolean reActivateIt() {
        ...
        if (!isSOTrx() && verificarAnular()) {
            ...
            MMatchInv[] mInv = MMatchInv.getInvoice(getCtx(), getC_Invoice_ID(), get_TrxName());
            for (int i = 0; i < mInv.length; i++) //	Desasocir cada Match con el recibo
            {
                mInv[i].delete(true);
            }
    ...
    public String getSummary() {
        ...
        sb.append(": ").
                append(Msg.translate(getCtx(), "GrandTotal")).append("=").append(getGrandTotal()).append(" (#").append(getLines(false).length).append(")");
        //	 - Description
                ...
                if (docType.getDocBaseType().equals("RRI")) {
                    ...
                    if (docType.getDocBaseType().equals("RRC")) {
                        ...
                        if (docType.getDocBaseType().equals("RPC")) {
                            actual = previo.add(getGrandTotal());
                        } else if (docType.getDocBaseType().equals("RPI")) {
                            actual = previo.subtract(getGrandTotal());
                        }
                    }
                }
