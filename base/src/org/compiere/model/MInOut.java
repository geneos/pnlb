...
public class MInOut extends X_M_InOut implements DocAction {
    ...
    public String prepareIt() {
        ...
        if (isReturn()) {
            for (int i = 0; i < lines.length; i++) {
                BigDecimal qtyReturn = lines[i].getQtyEntered().negate();
                int M_Locator_ID = getSalidasLocatorID();
                MStorage aStorage = MStorage.get(getCtx(), M_Locator_ID, lines[i].getM_Product_ID(), lines[i].getM_AttributeSetInstance_ID(), get_TrxName());
                if (aStorage == null || aStorage.getQtyAvailable().compareTo(qtyReturn) == -1) {
                    MProduct prod = new MProduct(getCtx(), lines[i].getM_Product_ID(), get_TrxName());
                    MAttributeSetInstance masi = new MAttributeSetInstance(getCtx(), lines[i].getM_AttributeSetInstance_ID(), get_TrxName());
                    m_processMsg = "No existe cantidad sufiente en Salidas para  producto: " + prod + " partida: " + masi.getDescription();
                    return DocAction.STATUS_Invalid;
                }
    ...
    public String completeIt() {
        ...
        if (!docType.getName().equals("Remito Loma Hermosa")
            ...
            for (int lineIndex = 0; lineIndex < lines.length; lineIndex++) {
                ...
                if (isReturn() && (sLine.getQtyEntered().compareTo(Env.ZERO) >= 0)) {
                    JOptionPane.showMessageDialog(null, "La cantidad en una devolución debe ser negativa.", "Salvando", JOptionPane.INFORMATION_MESSAGE);
                    return DocAction.STATUS_Invalid;
                }
                ...
                if (sLine.getC_OrderLine_ID() != 0) {
                    ...
                    if (!order.isSOTrx()) //	Para que tome la OC sin los cambios hechos por esta transaccion se pone null
                    {
                    ...
                    if (isSOTrx()) //QtySO = sLine.getMovementQty();
                    //BISion - 29/10/2009 - Santiago Ibañez
                    //COM-PAN-BUG-05.001.01
                    {
                        QtySO = sLine.getQtyEntered();
                    } else //QtyPO = sLine.getMovementQty();
                    //BISion - 29/10/2009 - Santiago Ibañez
                    //COM-PAN-BUG-05.001.01
                    {
                    ...
                    } else //Si esta recepcion + entregado no supere la cantidad actualizo ordenado con la cantidad del movimiento
                    {
                        QtyPO = sLine.getQtyEntered();
                    }
                }
    ...
    public boolean anularComprobantesAsociados() {
        ...
        for (int i = 0; i < mPO.length; i++) // Desasocir cada Match con la orden
        {
            mPO[i].delete(true);
        }
        ...
        for (int i = 0; i < mInv.length; i++) {	//	Anular remito de cada linea asociada, generalmente es solo uno.
        ...
        for (int i = 0; i < getLines().length; i++) {	//	Anular remito de cada linea asociada, generalmente es solo uno.
    ...
    public boolean reverseCorrectIt() {
        ...
        if (!isSOTrx()) {
            ...
            if (getC_Order_ID() != 0) //-	Desde una OV
    ...
    private int superaUmbral() {
        try {
            ...
            for (int i = 0; i < lines.length; i++) {
                ...
                if (qty.compareTo(qtyEntered.add(qtyEntered.multiply(u))) > 0) //retorno para informar el numero de linea que supera umbral
                {
    ...
    public boolean isReturn() {
        return (getC_DocType_ID() == 5000046);
    }

    public int getSalidasLocatorID() {
        return 1000296;
    }
}	//	MInOut