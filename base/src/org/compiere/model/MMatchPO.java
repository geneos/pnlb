...
public class MMatchPO extends X_M_MatchPO
{
	...
	protected boolean afterSave (boolean newRecord, boolean success)
	{
		...
		if (success && getC_OrderLine_ID() != 0)
		{
			...
			{
				MInvoiceLine ml = getInvoiceLine();
                                if (getC_InvoiceLine_ID() != 0) {
                                    MInvoice mi = new MInvoice(getCtx(), ml.getC_Invoice_ID(), get_TrxName());

                                    /*14-01-2010 Camarzana Mariano
                                     * cambiado mi.getC_DocType_ID() por mi.getC_DocTypeTarget_ID()
                                     * siempre que sea factura se debe preguntar por getC_DocTypeTarget_ID()
                                     */



                                    MDocType md = MDocType.get(getCtx(), mi.getC_DocTypeTarget_ID());
                                    if (!md.getDocBaseType().equals(MDocType.DOCBASETYPE_ARCreditMemo)){
     ...
     * Metodo realizado para retornar todas los MatchPO asociados a una linea
     * de OC dada.
     * @param ctx
     ...
     * @return
     */
    public static MMatchPO[] getByOrderLine (Properties ctx,
		int C_OrderLine_ID, String trxName)
	{
    	if (C_OrderLine_ID == 0)
			return new MMatchPO[]{};
        
    	/*08-08-2011 Camarzana Mariano
         * Se le agrego la restriccion de que tenga el identificador de remitos asociado 
         * ya que estoy buscando las recepciones de producto
         */ 
		String sql = "SELECT * FROM M_MatchPO WHERE C_OrderLine_ID=? " +
