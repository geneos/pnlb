 ...
 *	
 *  @author Jorg Janke
 *  @version $Id: MWFActivity.java,v 1.48 2005/12/13 00:15:27 jjanke Exp $
 */
public class MWFActivity extends X_AD_WF_Activity implements Runnable
{
	...
	public void run()
	{
		...
		{
			log.log(Level.SEVERE, "" + getNode(), e);
			/****	Trx Rollback	****/
			trx.rollback();
			trx.close();
			//
	...
	private boolean performWork (Trx trx) throws Exception
	{
		...
		else if (MWFNode.ACTION_AppsReport.equals(action))
		{
			...
			/*
                         ...
                         */
                        if (!note.save()){
                            log.log(Level.SEVERE, "No se pudo guardar la Nota: "+ m_node.getName(true));
                        }
			//	Attachment
			MAttachment attachment = new MAttachment (getCtx(), MNote.getTableId(MNote.Table_Name), note.getAD_Note_ID(), get_TrxName());
