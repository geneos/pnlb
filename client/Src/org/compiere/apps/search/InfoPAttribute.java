...
public class InfoPAttribute extends CDialog
{
	...
	private String createQuery()
	{
		...
		**/
		...
		// Added by Gunther Hoppe, 23.08.2005 add vpj-cd e-evolution
                // Begin
		String str = null;
                // End e-evolution
		//	Instance Editors
		for (int i = 0; i < m_instanceEditors.size(); i++)
		{
			...
			//	Add to where
			if (iAttr.length() > 0)
                        // Added by Gunther Hoppe, 23.08.2005
                        // Begin add vpj-cd e-evolution
                        {    
				...
				str = "asi.M_AttributeSetInstance_ID IN "
		...
		for (int i = 0; i < m_productEditors.size(); i++)
		{
			...
			//	Add to Where
			if (pAttr.length() > 0)
                        // Added by Gunther Hoppe, 23.08.2005 add vpj e-evolution 
                        // Begin 
                        {    
				...
				str = "p.M_AttributeSetInstance_ID IN "
					+"(SELECT M_AttributeSetInstance_ID "
					+"FROM M_AttributeInstance "
					+"WHERE "
					+pAttr.toString()
		...
		m_query = null;
		if (sb.length() > 0)
                // Bug fix by Gunther Hoppe, 13.07.2005
                // Begin
                {    
			sb.append(")");              
			m_query = sb.toString();
...
//	 Added by Gunther Hoppe, 23.08.2005 add e-evolution
//	 Begin
		
		private StringBuffer qtyOnBatchWhere;

		public static final String SQL_QTY_PATTERN = "SELECT sum({0}(p.M_Product_ID, asi.M_AttributeSetInstance_ID, {1}, 0))"
													+" FROM M_Storage s" 
													+" INNER JOIN M_AttributeSetInstance asi ON (s.M_AttributeSetInstance_ID=asi.M_AttributeSetInstance_ID)" 
													+" INNER JOIN M_Product p ON (p.M_AttributeSet_ID=asi.M_AttributeSet_ID)"
													+" WHERE asi.M_AttributeSetInstance_ID IN ("
													+" SELECT M_AttributeSetInstance_ID" 
													+" FROM M_AttributeInstance" 
													+" WHERE {2} )";
			
		protected String getSQLForQtyColumn(String function, int warehouseID) {
			
			return (qtyOnBatchWhere == null) ? null : java.text.MessageFormat.format(SQL_QTY_PATTERN, new Object[] {function, String.valueOf(warehouseID),qtyOnBatchWhere});		
		}

		private void addToQtyOnBatchWhere(String clause) {
			
			if(qtyOnBatchWhere == null) {
				
				qtyOnBatchWhere = new StringBuffer();
			}
			
			if(qtyOnBatchWhere.length() == 0) {
				
				qtyOnBatchWhere.append(clause);
			}
			else {
				
				qtyOnBatchWhere.append(" AND "+clause);
			}
		}
//	 End                
