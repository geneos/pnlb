 ...
 *
 * @author  Jorg Janke
 * @version $Id: VAllocation.java,v 1.56 2005/11/28 03:35:46 jjanke Exp $
 */
	...
	implements FormPanel, ActionListener, TableModelListener, VetoableChangeListener
{
	...
	private int         i_payment = 7;
        ...
        /*
         *  Zynnia 06/06/2012
         *  Los indices deben ser cambiados en caso de cambiar el orden y/o agregar columnas
         *  JF
         *
         *
         */
...
//	private int			i_multiplier = 10;
	...
	private void dynInit() throws Exception
	{
		...
		// Begin e-Evolution add ogi-cd 19Jul
			MLookup lookupBP = MLookupFactory.get (Env.getCtx(), m_WindowNo, 0, AD_Column_ID, DisplayType.Search);
			//MLookup lookupBP = MLookupFactory.get (Env.getCtx(), m_WindowNo, 0, AD_Column_ID, DisplayType.Search,language , "MPC_Order_ID" , 0 , false , "MPC_Order.DocStatus = '");
	    // End e-Evolution add ogi-cd 19Jul
	...
	private void loadBPartner ()
	{
		 ...
		 */
                 /*
                 *  Cambio en algunos atributos ya que el alias asignado no existia.
                 *  Por ende no arrojaba resultados esta consulta.
                 *  Zynnia 04/05/2012
                 */
		...
		StringBuffer sql = new StringBuffer("SELECT p.DateTrx,p.DocumentNo,p.C_Payment_ID,"  //  1..3
			...
			+ " AND p.C_BPartner_ID=? AND p.IsActive = 'Y'"
                        + " AND p.DocStatus = 'CO'");                   		//      #3
		...
		sql.toString(), "p", MRole.SQL_FULLYQUALIFIED, MRole.SQL_RO);
		log.finer(finalSql);
                // End e-Evolution
		...
		try
		{
                        // Begin e-Evolution vpj-cd 11 May 2006
                            ...
                            PreparedStatement pstmt = DB.prepareStatement(finalSql.toString(), null);
                        // End e-Evolution
...
//		paymentTable.setColumnClass(i++, BigDecimal.class, true);      	//  8-Multiplier
		 ...
		 */
                ...
                /*
                 *   Zynnia 04/06/2012
                 *   Modificación para que en la ventana Asig. de Comprobantes aparezca el tipo de documento en las facturas.
                 *
                 */
		...
		//	+ "(i.C_Invoice_ID,?,C_InvoicePaySchedule_ID),i.C_Currency_ID,?,i.DateInvoiced,i.C_ConversionType_ID,i.AD_Client_ID,i.AD_Org_ID)*i.Multiplier*i.MultiplierAP,"               //  #3, #4
			+ "i.MultiplierAP , i.C_DocType_ID, i.C_ConversionType_ID, i.AD_Client_ID, i.AD_Org_ID , i.Multiplier "  //7,8,9,10,11,12           // 9..10..11..12..13
			+ "FROM C_Invoice_v i"		//  corrected for CM/Split
			+ " INNER JOIN C_Currency c ON (i.C_Currency_ID=c.C_Currency_ID) "
			+ "WHERE i.IsPaid='N' AND i.Processed='Y'"
			+ " AND i.C_BPartner_ID=? AND i.IsActive = 'Y'"
                        + " AND i.DocStatus = 'CO'");                                            //  #5
		if (!multiCurrency.isSelected())
			sql.append(" AND i.C_Currency_ID=?");                                   //  #6

                // Begin e-Evolution vpj-cd 11 May 2006
              	log.finest(sql.toString());
		String finalSql2 = MRole.getDefault(Env.getCtx(), false).addAccessSQL(
		...
		try
		{
                    // Begin e-Evolution vpj-cd 11 May 2006
		    //PreparedStatement pstmt = DB.prepareStatement(sql.toString(), null);
                        PreparedStatement pstmt = DB.prepareStatement(finalSql2.toString(), null);
                    // End e-Evolution
			...
			while (rs.next())
			{
                                BigDecimal openInvoice = Invoice.open(rs.getInt(3), 0);
                                BigDecimal currencyConvertOpen = Currency.convert(openInvoice,m_C_Currency_ID,m_C_Currency_ID,rs.getTimestamp(1),rs.getInt(9),rs.getInt(10),rs.getInt(11)).multiply(rs.getBigDecimal(7));
                                BigDecimal discountInvoice = Invoice.discount(rs.getInt(3),(Timestamp)dateField.getValue(),0).multiply(rs.getBigDecimal(7)).multiply(rs.getBigDecimal(12));
                                StringBuffer sqlType = new StringBuffer("SELECT name FROM C_docType WHERE C_docType_ID = "+rs.getInt(8));
                                PreparedStatement pstmtType = DB.prepareStatement(sqlType.toString(), null);
				...
				line.add(rs.getTimestamp(1));       //  1-TrxDate
                                if (rsType.next()){
                                    line.add(rsType.getString(1));      //  2 - TypeDoc
                                }
                                KeyNamePair pp = new KeyNamePair(rs.getInt(3), rs.getString(2));
				line.add(pp);                       //  3-Value
				if (multiCurrency.isSelected())
				...
				BigDecimal open = currencyConvertOpen;//rs.getBigDecimal(7);
				if (open == null)		//	no conversion rate
					open = Env.ZERO;
				line.add(currencyConvertOpen);      				//  5/7-ConvOpen
				BigDecimal discount = discountInvoice;//rs.getBigDecimal(8);
				if (discount == null)	//	no concersion rate
					discount = Env.ZERO;
				line.add(discount);					//  6/8-ConvAllowedDisc
				line.add(Env.ZERO);      			//  7/9-WriteOff
				line.add(Env.ZERO);				    //  8/9-Applied
//				line.add(rs.getBigDecimal(9));		//	8/10-Multiplier
		...
		columnNames.add(Msg.getMsg(Env.getCtx(), "Select"));
		columnNames.add(Msg.translate(Env.getCtx(), "Date"));
                columnNames.add(Msg.translate(Env.getCtx(), "Tipo Doc."));
		columnNames.add(Msg.translate(Env.getCtx(), "DocumentNo"));
		if (multiCurrency.isSelected())
		{
			columnNames.add(Msg.getMsg(Env.getCtx(), "TrxCurrency"));
...
//		columnNames.add(" ");	//	Multiplier
		...
		invoiceTable.setColumnClass(i++, Timestamp.class, true);        //  1-TrxDate
                invoiceTable.setColumnClass(i++, String.class, true);           //  2 - TypeDoc
		invoiceTable.setColumnClass(i++, String.class, true);           //  3-Value
		if (multiCurrency.isSelected())
		{
...
//		invoiceTable.setColumnClass(i++, BigDecimal.class, true);      	//  11-Multiplier
		//  Table UI
		invoiceTable.autoSize();

		i_open = multiCurrency.isSelected() ? 7 : 5;
                i_open_payment = multiCurrency.isSelected() ? 6 : 4;
...
//		i_multiplier = multiCurrency.isSelected() ? 10 : 8;
	...
	public void tableChanged(TableModelEvent e)
	{
		...
		if (!isInvoice)
		{
			...
			if (col == 0)
			{
				...
				{
					BigDecimal amount = (BigDecimal)payment.getValueAt(row, i_open_payment);   //  Open Amount
	...
	private void saveData()
	{
		...
		for (int i = 0; i < iRows; i++)
		{
			...
			if (((Boolean)invoice.getValueAt(i, 0)).booleanValue())
			{
				...
				System.out.print("\nMonto= " + AppliedAmt.floatValue() + "; Descuento= " + DiscountAmt.floatValue()
								+ "; Ajuste= " + WriteOffAmt.floatValue() + "; SobreSub= " + OverUnderAmt.floatValue() + "\n");
				...
				for (int j = 0; j < paymentList.size() && AppliedAmt.signum() != 0; j++)
				{
					...
					if (PaymentAmt.signum() != 0)
					{
						...
						if (PaymentAmt.compareTo(AppliedAmt) == 1)
							amount = PaymentAmt;
						...
						//	Allocation Header
						if (alloc.get_ID() == 0 && !alloc.save(trx.getTrxName()))
						{
		...
		for (int i = 0; i < iRows; i++)
		{
			...
			if (((Boolean)invoice.getValueAt(i, 0)).booleanValue())
			{
				...
				BigDecimal open = DB.getSQLValueBD(trx.getTrxName(), sql, C_Invoice_ID);
                                ...
                                BigDecimal open;
                                try {
                                    open = Invoice.open(C_Invoice_ID, 0);
                                    ...
                                    Logger.getLogger(VAllocation.class.getName()).log(Level.SEVERE, null, ex);
                                }
		...
		for (int i = 0; i < paymentList.size(); i++)
		{
			...
			if (pay.testAllocation())
				if ( !pay.save(trx.getTrxName()) ){
                                    log.log(Level.SEVERE, "Payment ID: " + C_Payment_ID + " not Saved");
                                    JOptionPane.showMessageDialog(null,"Pago: "+pay.getDocumentNo()+ " no pudo guardar bien sus cambios, por favor verifique que la casilla de Asignado del mismo tenga el valor correcto", "Info", JOptionPane.INFORMATION_MESSAGE);
                                }
     ...
     public void setC_BPartner_ID(int C_BPartner_ID) {
         setC_BPartner_ID(C_BPartner_ID, false);
     }

    public void setC_BPartner_ID(int C_BPartner_ID, boolean execSearch) {
        bpartnerSearch.setValue(new Integer(C_BPartner_ID));
    ...
    public void setC_Currency_ID(int C_Currency_ID) {
        setC_Currency_ID(C_Currency_ID, false);
    }

    public void setC_Currency_ID(int C_Currency_ID, boolean execSearch) {
        currencyPick.setValue(new Integer(C_Currency_ID));
    ...
    public void setDate(Timestamp date, boolean execSearch) {
	...
	/*	COMENTADO POR DANIEL... METODO ORIGINAL
	 *
	 *
	 *
	 *
