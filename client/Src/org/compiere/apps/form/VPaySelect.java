 ...
 *
 *  @author Jorg Janke
 *  @version $Id: VPaySelect.java,v 1.50 2005/12/27 06:18:37 jjanke Exp $
 */
	...
	implements FormPanel, ActionListener, TableModelListener, ASyncProcess
{
	...
	private void dynInit()
	{
		...
		sql = MRole.getDefault().addAccessSQL(
			...
			+ " AND EXISTS (SELECT * FROM C_Invoice i WHERE bp.C_BPartner_ID=i.C_BPartner_ID"
                        //fjviejo e-evolution bodevidrio
			  //+ " AND i.IsSOTrx='N' AND i.IsPaid<>'Y') "
                        + " AND i.IsSOTrx='N' AND i.IsPaid<>'Y' AND i.DocStatus IN ('CO','CL')) "
	...
	//	Trx trx = Trx.get(trxName, true);	trx needs to be committed too
		...
		ProcessInfo pi = new ProcessInfo (m_frame.getTitle(), 155,
			MPaySelection.getTableId(MPaySelection.Table_Name), m_ps.getC_PaySelection_ID());
