...
public class Version implements Serializable, Cloneable, Comparable<Version> {
	...
	/**
	 * Gets the string that was parsed to create this Version object. This string may not accurately reflect the current values of the
	 * Version's components.
	 *
	 * @return The string that was parsed to create this Version object. This string may not accurately reflect the current values of the
	 *         Version's components.
	 */
	...
	public static Version parse(String toParse) {
        if (toParse == null) {
        	log.log(Level.SEVERE, String.format("Error can't convert null to object Version"));
            return null;
        }
        Matcher m = STD_VERSION_PATT.matcher(toParse);

		if (!m.find()) {
			log.log(Level.SEVERE, String.format("Error parsing version from '%s'", toParse));
            return null;
		}
		...
		if (isNotEmpty(m.group(2))) {
			v.setMajor(m.group(2));
		}

		if (isNotEmpty(m.group(3))) {
	 ...
	 *
	 * @return The number of components that make up the version. The value will always be between 1 (inclusive) and 4 (inclusive).
	 */
	 ...
	 *
	 * @param toSet The number of components that make up the version. Values less than 1 are treated as 1. Values greater than 4 are
	 *            treated as 4.
	 */
	...
	public String toStringWithPrefixAndSuffix() {
		return String.format("%s%s.%s.%s.%s%s", this.prefix == null ? "" : this.prefix, this.major, this.minor, this.build, this.revision,
	 ...
	 *
	 * @param components The number of components. Values less than 1 will be treated as 1 and values greater than 4 will be treated as 4.
	...
	public String toString(int components) {
		...
		switch (components) {
			...
			case 4: {
				buff.append(String.format(".%s.%s.%s", this.minor, this.build, this.revision));
				break;
			}
		}
	 ...
	 *
	 * @param components The number of components. Values less than 1 will be treated as 1 and values greater than 4 will be treated as 4.
	 * @return The version as a string including the prefix and suffix using the specified number of components.
	 */
