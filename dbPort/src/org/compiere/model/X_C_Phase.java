...
public class X_C_Phase extends PO {
	...
	public X_C_Phase(Properties ctx, int C_Phase_ID, String trxName) {
		...
		/**
		 * if (C_Phase_ID == 0) { setC_Phase_ID (0); setC_ProjectType_ID (0);
		 * setName (null); setSeqNo (0); //
		 * 
		 * @SQL=SELECT NVL(MAX(SeqNo),0)+10 AS DefaultValue FROM C_Phase WHERE
		 *             C_ProjectType_ID=@C_ProjectType_ID@ setStandardQty
		 *             (Env.ZERO); // 1 }
		 */
	}
	...
	public String toString() {
		StringBuffer sb = new StringBuffer("X_C_Phase[").append(get_ID())
				.append("]");
		return sb.toString();
	}
