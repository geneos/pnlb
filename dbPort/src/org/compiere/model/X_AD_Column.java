 ...
 * @version Release 2.5.3b - 2008-04-04 14:05:56.312
 */
public class X_AD_Column extends PO {
	...
	public X_AD_Column(Properties ctx, int AD_Column_ID, String trxName) {
		...
		/**
		 * if (AD_Column_ID == 0) { setAD_Column_ID (0); setAD_Element_ID (0);
		 * setAD_Reference_ID (0); setAD_Table_ID (0); setColumnName (null);
		 * setEntityType (null); // U setIsAlwaysUpdateable (false); // N
		 * setIsEncrypted (null); // N setIsIdentifier (false); setIsKey
		 * (false); setIsMandatory (false); setIsParent (false);
		 * setIsSelectionColumn (false); setIsTranslated (false);
		 * setIsUpdateable (true); // Y setName (null); setVersion (Env.ZERO); }
		 */
	}
	...
	public String toString() {
		StringBuffer sb = new StringBuffer("X_AD_Column[").append(get_ID())
				.append("]");
		return sb.toString();
	}
	...
	/**
	 * Set Always Updateable. The column is always updateable, even if the
	 * record is not active or processed
	 */
	...
	/**
	 * Get Always Updateable. The column is always updateable, even if the
	 * record is not active or processed
	 */
	...
	public void setIsEncrypted(String IsEncrypted) {
		if (IsEncrypted == null)
			throw new IllegalArgumentException("IsEncrypted is mandatory");
		if (IsEncrypted.equals("N") || IsEncrypted.equals("Y"))
			;
		else
			throw new IllegalArgumentException("IsEncrypted Invalid value - "
					+ IsEncrypted + " - Reference_ID=354 - N - Y");
	...
	/**
	 * Set Parent link column. This column is a link to the parent table (e.g.
	 * header from lines) - incl. Association key columns
	 */
	...
	/**
	 * Get Parent link column. This column is a link to the parent table (e.g.
	 * header from lines) - incl. Association key columns
	 */
	...
	/**
	 * Set Synchronize Database. Change database table definition when changing
	...
	/**
	 * Get Synchronize Database. Change database table definition when changing
	 * dictionary definition
	 */
