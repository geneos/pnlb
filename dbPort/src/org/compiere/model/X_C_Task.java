...
public class X_C_Task extends PO {
	...
	public X_C_Task(Properties ctx, int C_Task_ID, String trxName) {
		...
		/**
		 * if (C_Task_ID == 0) { setC_Phase_ID (0); setC_Task_ID (0); setName
		 * (null); setSeqNo (0); //
		 * 
		 * @SQL=SELECT NVL(MAX(SeqNo),0)+10 AS DefaultValue FROM C_Task WHERE
		 *             C_Phase_ID=@C_Phase_ID@ setStandardQty (Env.ZERO); // 1 }
		 */
	}
	...
	public String toString() {
		StringBuffer sb = new StringBuffer("X_C_Task[").append(get_ID())
				.append("]");
		return sb.toString();
	}
