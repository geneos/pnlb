 ...
 * @version Release 2.5.3b - 2008-04-04 14:06:04.796
 */
public class X_S_TimeExpenseLine extends PO {
	...
	public X_S_TimeExpenseLine(Properties ctx, int S_TimeExpenseLine_ID,
		...
		/**
		 * if (S_TimeExpenseLine_ID == 0) { setDateExpense (new
		 * Timestamp(System.currentTimeMillis())); //
		 * 
		 * @DateExpense@;
		 * @DateReport@ setIsInvoiced (false); setIsTimeReport (false); setLine
		 *              (0); //
		 * @SQL=SELECT NVL(MAX(Line),0)+10 AS DefaultValue FROM
		 *             S_TimeExpenseLine WHERE
		 *             S_TimeExpense_ID=@S_TimeExpense_ID@ setProcessed (false);
		 *             setS_TimeExpenseLine_ID (0); setS_TimeExpense_ID (0); }
		 */
	}
	...
	public String toString() {
		StringBuffer sb = new StringBuffer("X_S_TimeExpenseLine[").append(
				get_ID()).append("]");
		return sb.toString();
	}
	...
	public void setC_ProjectPhase_ID(int C_ProjectPhase_ID) {
		if (C_ProjectPhase_ID <= 0)
			set_Value("C_ProjectPhase_ID", null);
		else
	...
	public void setC_ProjectTask_ID(int C_ProjectTask_ID) {
		if (C_ProjectTask_ID <= 0)
			set_Value("C_ProjectTask_ID", null);
		else
	...
	/**
	 * Set Price Invoiced. The priced invoiced to the customer (in the currency
	 * of the customer's AR price list) - 0 for default price
	 */
	...
	/**
	 * Get Price Invoiced. The priced invoiced to the customer (in the currency
	 * of the customer's AR price list) - 0 for default price
	 */
	...
	/**
	 * Set Price Reimbursed. The reimbursed price (in currency of the employee's
	 * AP price list)
	 */
	...
	/**
	 * Get Price Reimbursed. The reimbursed price (in currency of the employee's
	 * AP price list)
	 */
	...
	public void setS_TimeExpenseLine_ID(int S_TimeExpenseLine_ID) {
		if (S_TimeExpenseLine_ID < 1)
			throw new IllegalArgumentException(
					"S_TimeExpenseLine_ID is mandatory.");
		set_ValueNoCheck("S_TimeExpenseLine_ID", new Integer(
