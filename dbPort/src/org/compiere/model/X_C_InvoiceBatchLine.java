 ...
 * @version Release 2.5.3b - 2008-04-04 14:05:59.531
 */
public class X_C_InvoiceBatchLine extends PO {
	...
	public X_C_InvoiceBatchLine(Properties ctx, int C_InvoiceBatchLine_ID,
		...
		/**
		 * if (C_InvoiceBatchLine_ID == 0) { setC_BPartner_ID (0); //
		 * 
		 * @C_BPartner_ID@ setC_BPartner_Location_ID (0); //
		 * @C_BPartner_Location_ID@ setC_Charge_ID (0); setC_DocType_ID (0); //
		 * @C_DocType_ID@ setC_InvoiceBatchLine_ID (0); setC_InvoiceBatch_ID
		 *                (0); setC_Tax_ID (0); setDateAcct (new
		 *                Timestamp(System.currentTimeMillis())); //
		 * @DateAcct@;
		 * @DateDoc@ setDateInvoiced (new
		 *           Timestamp(System.currentTimeMillis())); //
		 * @DateInvoiced@;
		 * @DateDoc@ setDocumentNo (null); //
		 * @DocumentNo@ setIsTaxIncluded (false); //
		 * @IsTaxIncluded@ setLine (0); //
		 * @SQL=SELECT NVL(MAX(Line),0)+10 AS DefaultValue FROM
		 *             C_InvoiceBatchLine WHERE
		 *             C_InvoiceBatch_ID=@C_InvoiceBatch_ID@ setLineNetAmt
		 *             (Env.ZERO); setLineTotalAmt (Env.ZERO); setPriceEntered
		 *             (Env.ZERO); setProcessed (false); setQtyEntered
	...
	public String toString() {
		StringBuffer sb = new StringBuffer("X_C_InvoiceBatchLine[").append(
				get_ID()).append("]");
		return sb.toString();
	}
	...
	public void setC_InvoiceBatchLine_ID(int C_InvoiceBatchLine_ID) {
		if (C_InvoiceBatchLine_ID < 1)
			throw new IllegalArgumentException(
					"C_InvoiceBatchLine_ID is mandatory.");
		set_ValueNoCheck("C_InvoiceBatchLine_ID", new Integer(
