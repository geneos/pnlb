 ...
 * @version Release 2.5.3b - 2008-04-04 14:06:00.281
 */
public class X_C_ProjectLine extends PO {
	...
	public X_C_ProjectLine(Properties ctx, int C_ProjectLine_ID, String trxName) {
		...
		/**
		 * if (C_ProjectLine_ID == 0) { setC_ProjectLine_ID (0); setC_Project_ID
		 * (0); setInvoicedAmt (Env.ZERO); setInvoicedQty (Env.ZERO); // 0
		 * setIsPrinted (true); // Y setLine (0); //
		 * 
		 * @SQL=SELECT NVL(MAX(Line),0)+10 AS DefaultValue FROM C_ProjectLine
		 *             WHERE C_Project_ID=@C_Project_ID@ setPlannedAmt
		 *             (Env.ZERO); setPlannedMarginAmt (Env.ZERO);
		 *             setPlannedPrice (Env.ZERO); setPlannedQty (Env.ZERO); //
		 *             1 setProcessed (false); // N }
		 */
	}
	...
	public String toString() {
		StringBuffer sb = new StringBuffer("X_C_ProjectLine[").append(get_ID())
				.append("]");
		return sb.toString();
	}
	...
	public void setC_OrderPO_ID(int C_OrderPO_ID) {
		if (C_OrderPO_ID <= 0)
			set_ValueNoCheck("C_OrderPO_ID", null);
		else
	...
	public void setC_ProjectLine_ID(int C_ProjectLine_ID) {
		if (C_ProjectLine_ID < 1)
			throw new IllegalArgumentException("C_ProjectLine_ID is mandatory.");
	...
	public void setPlannedPrice(BigDecimal PlannedPrice) {
		if (PlannedPrice == null)
			throw new IllegalArgumentException("PlannedPrice is mandatory.");
