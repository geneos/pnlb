...
public class X_M_BOMProduct extends PO {
	...
	public X_M_BOMProduct(Properties ctx, int M_BOMProduct_ID, String trxName) {
		...
		/**
		 * if (M_BOMProduct_ID == 0) { setBOMProductType (null); // S setBOMQty
		 * (Env.ZERO); // 1 setIsPhantom (false); setLeadTimeOffset (0); setLine
		 * (0); //
		 * 
		 * @SQL=SELECT NVL(MAX(Line),0)+10 AS DefaultValue FROM M_BOMProduct
		 *             WHERE M_BOM_ID=@M_BOM_ID@ setM_BOMProduct_ID (0);
		 *             setM_BOM_ID (0); }
		 */
	}
	...
	public String toString() {
		StringBuffer sb = new StringBuffer("X_M_BOMProduct[").append(get_ID())
				.append("]");
		return sb.toString();
	}
	...
	public void setBOMProductType(String BOMProductType) {
		if (BOMProductType == null)
			throw new IllegalArgumentException("BOMProductType is mandatory");
		if (BOMProductType.equals("A") || BOMProductType.equals("D")
				|| BOMProductType.equals("O") || BOMProductType.equals("S")
				|| BOMProductType.equals("X"))
			;
		else
			throw new IllegalArgumentException(
					"BOMProductType Invalid value - " + BOMProductType
							+ " - Reference_ID=349 - A - D - O - S - X");
	...
	public void setM_BOMAlternative_ID(int M_BOMAlternative_ID) {
		if (M_BOMAlternative_ID <= 0)
			set_Value("M_BOMAlternative_ID", null);
		else
	...
	public void setM_ProductBOM_ID(int M_ProductBOM_ID) {
		if (M_ProductBOM_ID <= 0)
			set_Value("M_ProductBOM_ID", null);
		else
	...
	public void setM_ProductOperation_ID(int M_ProductOperation_ID) {
		if (M_ProductOperation_ID <= 0)
			set_Value("M_ProductOperation_ID", null);
		else
			set_Value("M_ProductOperation_ID", new Integer(
