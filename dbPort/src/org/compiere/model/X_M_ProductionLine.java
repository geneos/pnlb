...
public class X_M_ProductionLine extends PO {
	...
	public X_M_ProductionLine(Properties ctx, int M_ProductionLine_ID,
		 ...
		 * 
		 * @SQL=SELECT NVL(MAX(Line),0)+10 AS DefaultValue FROM M_ProductionLine
		 *             WHERE M_ProductionPlan_ID=@M_ProductionPlan_ID@
		 *             setM_AttributeSetInstance_ID (0); setM_Locator_ID (0); //
		 * @M_Locator_ID@ setM_Product_ID (0); setM_ProductionLine_ID (0);
		 *                setM_ProductionPlan_ID (0); setMovementQty (Env.ZERO);
		 *                setProcessed (false); }
		 */
	}
	...
	public String toString() {
		StringBuffer sb = new StringBuffer("X_M_ProductionLine[").append(
				get_ID()).append("]");
		return sb.toString();
	}
