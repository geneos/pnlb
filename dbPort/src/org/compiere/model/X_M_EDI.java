 ...
 * @version Release 2.5.3b - 2008-04-04 14:06:02.718
 */
public class X_M_EDI extends PO {
	...
	public X_M_EDI(Properties ctx, int M_EDI_ID, String trxName) {
		...
		/**
		 * if (M_EDI_ID == 0) { setC_BP_EDI_ID (0); setDocumentNo (null);
		 * setEDIStatus (null); setLine (0); setM_EDI_ID (0); setM_Product_ID
		 * (0); setM_Warehouse_ID (0); setProcessed (false); setRequest_Qty
		 * (Env.ZERO); setRequest_Shipdate (new
		 * Timestamp(System.currentTimeMillis())); setTrxSent (new
		 * Timestamp(System.currentTimeMillis())); setTrxType (null); }
		 */
	}
	...
	public void setEDIStatus(String EDIStatus) {
		if (EDIStatus == null)
			throw new IllegalArgumentException("EDIStatus is mandatory");
		if (EDIStatus.equals("A") || EDIStatus.equals("C")
				|| EDIStatus.equals("D") || EDIStatus.equals("S"))
			;
		else
			throw new IllegalArgumentException("EDIStatus Invalid value - "
					+ EDIStatus + " - Reference_ID=202 - A - C - D - S");
