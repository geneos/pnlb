...
public class X_K_Type extends PO {
	...
	public X_K_Type(Properties ctx, int K_Type_ID, String trxName) {
		super(ctx, K_Type_ID, trxName);
		/**
		 * if (K_Type_ID == 0) { setIsPublic (true); // Y setIsPublicWrite
		 * (true); // Y setK_Type_ID (0); setName (null); }
		 */
	}

	/** Load Constructor */
	public X_K_Type(Properties ctx, ResultSet rs, String trxName) {
	...
	public String toString() {
		StringBuffer sb = new StringBuffer("X_K_Type[").append(get_ID())
				.append("]");
		return sb.toString();
	}
