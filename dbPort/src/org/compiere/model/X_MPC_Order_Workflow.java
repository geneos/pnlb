...
/**
 * Generated Model for MPC_Order_Workflow
 * 
 * @author Jorg Janke (generated)
 * @version Release 2.5.3b - 2008-04-04 14:06:02.109
 */
public class X_MPC_Order_Workflow extends PO {
	/** Standard Constructor */
	public X_MPC_Order_Workflow(Properties ctx, int MPC_Order_Workflow_ID,
			String trxName) {
		super(ctx, MPC_Order_Workflow_ID, trxName);
		/**
		 * if (MPC_Order_Workflow_ID == 0) { setAD_Workflow_ID (0);
		 * setAccessLevel (null); setAuthor (null); setCost (0); setDuration
		 * (0); // 0 setDurationUnit (null); // h setEntityType (null); setLimit
		 * (0); setMPC_Order_ID (0); setMPC_Order_Workflow_ID (0); setName
		 * (null); setPriority (0); setPublishStatus (null); // U
		 * setValidateWorkflow (null); setVersion (0); setWaitingTime (0); }
		 */
	}
	...
	public String toString() {
		StringBuffer sb = new StringBuffer("X_MPC_Order_Workflow[").append(
				get_ID()).append("]");
		return sb.toString();
	}
	...
	public void setDurationUnit(String DurationUnit) {
		...
		if (DurationUnit.equals("D") || DurationUnit.equals("M")
				|| DurationUnit.equals("Y") || DurationUnit.equals("h")
				|| DurationUnit.equals("m") || DurationUnit.equals("s"))
			;
		else
			throw new IllegalArgumentException("DurationUnit Invalid value - "
					+ DurationUnit
					+ " - Reference_ID=299 - D - M - Y - h - m - s");
