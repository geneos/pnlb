DROP TABLE MPC_Order_Node_Trl;
CREATE TABLE MPC_Order_Node_Trl(
    ...
    CONSTRAINT mpc_order_node_trl_Key PRIMARY KEY (MPC_Order_Node_ID  , AD_Language) ,
    CONSTRAINT ad_language_ordernodetrl FOREIGN KEY (ad_language) REFERENCES ad_language (ad_language) ON DELETE CASCADE,
    CONSTRAINT mpc_ordernodetrl FOREIGN KEY (mpc_order_node_id, mpc_order_id) REFERENCES mpc_order_node (mpc_order_node_id, mpc_order_id)  ON DELETE CASCADE
);
DROP TABLE MPC_Order_Workflow_Trl;
CREATE TABLE MPC_Order_Workflow_Trl
(
  ...
  CONSTRAINT ad_language_order_wf_trl FOREIGN KEY (ad_language) REFERENCES ad_language (ad_language) ON DELETE CASCADE,
  CONSTRAINT mpc_order_workflowtrl FOREIGN KEY (mpc_order_workflow_id, mpc_order_id) REFERENCES mpc_order_workflow (mpc_order_workflow_id, mpc_order_id) ON DELETE CASCADE
) ;
...
create or replace view compiere.mpc_order_receipt_issue as
select
	obl.mpc_order_bomline_id, 
	obl.iscritical, 
	p.value, 
