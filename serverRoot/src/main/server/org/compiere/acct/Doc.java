 ...
 *
 *  C_MovimientoFondos: AMM
 *      Doc_MovFondos   259 - DocType fixed
 *	
 *  </pre>
 *  @author Jorg Janke
...
public abstract class Doc {
    ...
    public static int[] documentsTableID = new int[]{
        MInvoice.getTableId(MInvoice.Table_Name), // C_Invoice								-	318
        MAllocationHdr.getTableId(MAllocationHdr.Table_Name), // C_Allocation				-	735
        MCash.getTableId(MCash.Table_Name), // C_Cash										-	407
        MBankStatement.getTableId(MBankStatement.Table_Name), // C_BankStatement			-	392
        // MOrder.Table_ID, // C_Order
        MPayment.getTableId(MPayment.Table_Name), // C_Payment								-	335
        // MInOut.Table_ID, // M_InOut
        MInventory.getTableId(MInventory.Table_Name), // M_Inventory						-	321
        MMovement.getTableId(MMovement.Table_Name), // M_Movement							-	323
        X_M_Production.getTableId(X_M_Production.Table_Name), // M_Production				-	325
        MJournal.getTableId(MJournal.Table_Name), // GL_Journal								-	224
        MMatchInv.getTableId(MMatchInv.Table_Name), // M_MatchInv							-	472
        MMatchPO.getTableId(MMatchPO.Table_Name), // M_MatchPO								-	473
        MProjectIssue.getTableId(MProjectIssue.Table_Name), // C_ProjectIssue				-	623
        // MRequisition.Table_ID // M_Requisition
        MMOVIMIENTOFONDOS.getTableId(MMOVIMIENTOFONDOS.Table_Name) // C_MovimientoFondos	-	1000304
    };
    /*public static int[] documentsTableID = new int[] {
    318, // C_Invoice				-	318
    735, // C_Allocation			-	735
    407, // C_Cash					-	407
    392, // C_BankStatement			-	392
    335, // C_Payment				-	335
    321, // M_Inventory				-	321
    323, // M_Movement				-	323
    325, // M_Production			-	325
    224, // GL_Journal				-	224
    472, // M_MatchInv				-	472
    473, // M_MatchPO				-	473
    623, // C_ProjectIssue			-	623
    1000304 // C_MovimientoFondos	-	1000304
    };*/
    ...
    public static String[] documentsTableName = new String[]{
        ...
        MMOVIMIENTOFONDOS.Table_Name //	C_MovimientoFondos
    };
    /**
     * ACTUALIZADO POR BISion - 01/12/08
     * 		DANIEL GINI - ANULAR CONTABILIDAD AUTOMATICA DE DOCUMENTOS
     */
    /**************************************************************************
     * 	 Document Types
     *  --------------
    ...
    /**	AR Credit Memo          */
    public static final String DOCTYPE_ARCredit_Rev = "RRC";
    /**	AR Receipt              */
    public static final String DOCTYPE_ARInvoice_Rev = "RRI";
    /**	AP Invoices             */
    public static final String DOCTYPE_APInvoice_Rev = "RPI";
    /**	AP Credit Memo          */
    public static final String DOCTYPE_APCredit_Rev = "RPC";
    /**	AP Payment              */
    ...
    public static final String DOCTYPE_MovFondos = "AMM";
    ...
    public static Doc get(MAcctSchema[] ass, int AD_Table_ID, ResultSet rs, String trxName) {
        ...
        if (AD_Table_ID == MCash.getTableId(MCash.Table_Name)) {
            doc = new Doc_Cash(ass, rs, trxName);
        }

        if (AD_Table_ID == MBankStatement.getTableId(MBankStatement.Table_Name)) {
            doc = new Doc_Bank(ass, rs, trxName);
        }

        if (AD_Table_ID == MOrder.getTableId(MOrder.Table_Name)) {
        ...
        if (AD_Table_ID == MInOut.getTableId(MInOut.Table_Name)) {
            doc = new Doc_InOut(ass, rs, trxName);
        }

        if (AD_Table_ID == MInventory.getTableId(MInventory.Table_Name)) {
            doc = new Doc_Inventory(ass, rs, trxName);
        }

        if (AD_Table_ID == MMovement.getTableId(MMovement.Table_Name)) {
            doc = new Doc_Movement(ass, rs, trxName);
        }

        if (AD_Table_ID == X_M_Production.getTableId(X_M_Production.Table_Name)) {
        ...
        if (AD_Table_ID == MMatchInv.getTableId(MMatchInv.Table_Name)) {
            doc = new Doc_MatchInv(ass, rs, trxName);
        }

        if (AD_Table_ID == MMatchPO.getTableId(MMatchPO.Table_Name)) {
            doc = new Doc_MatchPO(ass, rs, trxName);
        }

        if (AD_Table_ID == MProjectIssue.getTableId(MProjectIssue.Table_Name)) {
            doc = new Doc_ProjectIssue(ass, rs, trxName);
        }

        if (AD_Table_ID == MMOVIMIENTOFONDOS.getTableId(MMOVIMIENTOFONDOS.Table_Name)) {
            doc = new Doc_MovFondos(ass, rs, trxName);
        }

        if (AD_Table_ID == MRequisition.getTableId(MRequisition.Table_Name)) {
...
/*
    ...
    public final String post(boolean force, boolean repost) {
        ...
        /*
         *  Modificación para no generar los asientos segun la fecha anterior a la registrada opr base.
         *  Zynnia 20/03/2012
         * 
         *  En base al tipo de documento busco en la tabla de C_ACCTINI para saber fecha base desde la
         *  que no tengo que generar registros contables hacia atras.
         * 
         */



        String sqlDateIni = "SELECT INIDATE FROM C_ACCTINI WHERE C_DOCTYPE_ID = " + this.getC_DocType_ID();

        System.out.println("Validacion en Post de Doc con " + sqlDateIni);

        Timestamp iniDateAcct = null;

        try {

            PreparedStatement pstmt = null;
            pstmt = DB.prepareStatement(sqlDateIni, null);
            ResultSet rs = pstmt.executeQuery();
            while (rs.next()) {
                iniDateAcct = rs.getTimestamp(1);
                System.out.println("Fecha de validación en Post de Doc con " + iniDateAcct);
            }
            rs.close();
            pstmt.close();

            if (!(iniDateAcct == null)) {
                System.out.println("<> de null " + iniDateAcct + " vs " + this.getDateAcct());
                if (this.getDateAcct().compareTo(iniDateAcct) <= 0) {
                    log.log(Level.SEVERE, this.getDateAcct() + " - Fecha de documento menor al parámetro no genera asientos");
                    System.out.println("Fecha menor al parametro");
                    return null;
                }
            }
            System.out.println("verificacion antes de <> de null " + iniDateAcct);

        } catch (Exception e) {
            log.log(Level.SEVERE, "DocType date acct initial error", e);
            return STATUS_Error;
        }
    ...
    private final String postLogic(int index) {
        ...
        for (int f = 0; f < facts.size(); f++) {
            ...
            if (!fact.checkAccounts()) {
                String sqlDelete = "DELETE Fact_Acct WHERE AD_Table_ID = " + get_Table_ID();
                sqlDelete += " AND Record_ID = " + p_po.get_ID();
                int no = DB.executeUpdate(sqlDelete, getTrxName());
                if (no != 0) {
                    log.info("deleted=" + no);
                }
    ...
    public static final int ACCTTYPE_C_MExterno = 5;
    public static final int ACCTTYPE_V_MExterno = 6;
    ...
    public static final int ACCTTYPE_WithholdingPaymentGan = 120;
    public static final int ACCTTYPE_WithholdingPaymentIB = 121;
    public static final int ACCTTYPE_WithholdingPaymentIVA = 122;
    public static final int ACCTTYPE_WithholdingPaymentSUSS = 123;
    public static final int ACCTTYPE_WithholdingPaymentOTHER = 124;
    public static final int ACCTTYPE_PercepcionVentaIVA = 125;
    public static final int ACCTTYPE_PercepcionVentaIB = 126;
    public static final int ACCTTYPE_PercepcionVentaOTHER = 127;
    /** Account Type - Cash  Default  (DANIEL GINI - Contabilidad Cobranza) */
    public static final int ACCTTYPE_CashAssetDefault = 128;
    public static final int ACCTTYPE_CashTransferDefault = 129;
    public static final int ACCTTYPE_CashExpenseDefault = 130;
    public static final int ACCTTYPE_CashReceiptDefault = 131;
    public static final int ACCTTYPE_CashDifferenceDefault = 132;
    ...
    public int getValidCombination_ID(int AcctType, MAcctSchema as) {
        ...
        if (AcctType == ACCTTYPE_Charge) //	see getChargeAccount in DocLine
        {
            ...
            para_1 = getC_BPartner_ID();
        } else if (AcctType == ACCTTYPE_C_MExterno) {
            sql = "SELECT C_MExterno_Acct FROM C_BP_Customer_Acct WHERE C_BPartner_ID=? AND C_AcctSchema_ID=?";
            para_1 = getC_BPartner_ID();
        } else if (AcctType == ACCTTYPE_V_MExterno) {
            sql = "SELECT V_MExterno_Acct FROM C_BP_Vendor_Acct WHERE C_BPartner_ID=? AND C_AcctSchema_ID=?";
            para_1 = getC_BPartner_ID();
        ...
        else if (AcctType == ACCTTYPE_CashAssetDefault) {
            sql = "SELECT CB_Asset_Acct FROM C_AcctSchema_Default WHERE C_AcctSchema_ID=?";
            para_1 = -1;
        } else if (AcctType == ACCTTYPE_CashTransferDefault) {
            sql = "SELECT CB_CashTransfer_Acct FROM C_AcctSchema_Default WHERE C_AcctSchema_ID=?";
            para_1 = -1;
        } else if (AcctType == ACCTTYPE_CashExpenseDefault) {
            sql = "SELECT CB_Expense_Acct FROM C_AcctSchema_Default WHERE C_AcctSchema_ID=?";
            para_1 = -1;
        } else if (AcctType == ACCTTYPE_CashReceiptDefault) {
            sql = "SELECT CB_Receipt_Acct FROM C_AcctSchema_Default WHERE C_AcctSchema_ID=?";
            para_1 = -1;
        } else if (AcctType == ACCTTYPE_CashDifferenceDefault) {
            sql = "SELECT CB_Differences_Acct FROM C_AcctSchema_Default WHERE C_AcctSchema_ID=?";
            para_1 = -1;
        } /** Inventory Accounts          */
         ...
         * ACTUALIZADO POR BISion - 25/11/08
         * 		DANIEL GINI - WithholdingPayment Accounts
         */
        else if (AcctType == ACCTTYPE_WithholdingPaymentGan) {
            sql = "SELECT WPAY_GAIN_ACCT FROM C_AcctSchema_Default WHERE C_AcctSchema_ID=?";
            para_1 = -1;
        } else if (AcctType == ACCTTYPE_WithholdingPaymentIB) {
            sql = "SELECT WPAY_IIBB_ACCT FROM C_AcctSchema_Default WHERE C_AcctSchema_ID=?";
            para_1 = -1;
        } else if (AcctType == ACCTTYPE_WithholdingPaymentIVA) {
            sql = "SELECT WPAY_IVA_ACCT FROM C_AcctSchema_Default WHERE C_AcctSchema_ID=?";
            para_1 = -1;
        } else if (AcctType == ACCTTYPE_WithholdingPaymentSUSS) {
            sql = "SELECT WPAY_SUSS_ACCT FROM C_AcctSchema_Default WHERE C_AcctSchema_ID=?";
            para_1 = -1;
        } else if (AcctType == ACCTTYPE_WithholdingPaymentOTHER) {
            sql = "SELECT WPAY_OTHER_ACCT FROM C_AcctSchema_Default WHERE C_AcctSchema_ID=?";
            para_1 = -1;
        } /**
         * ACTUALIZADO POR BISion - 11/12/08
         * 		DANIEL GINI - PercepcionVenta Accounts
         */
        else if (AcctType == ACCTTYPE_PercepcionVentaIVA) {
            sql = "SELECT PPAY_IVA_ACCT FROM C_AcctSchema_Default WHERE C_AcctSchema_ID=?";
            para_1 = -1;
        } else if (AcctType == ACCTTYPE_PercepcionVentaIB) {
            sql = "SELECT PPAY_IIBB_ACCT FROM C_AcctSchema_Default WHERE C_AcctSchema_ID=?";
            para_1 = -1;
        } else if (AcctType == ACCTTYPE_PercepcionVentaOTHER) {
            sql = "SELECT PPAY_OTHER_ACCT FROM C_AcctSchema_Default WHERE C_AcctSchema_ID=?";
            para_1 = -1;
        } else {
