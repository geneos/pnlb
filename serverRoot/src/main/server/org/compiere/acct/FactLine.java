...
public final class FactLine extends X_Fact_Acct
{
	...
	{
                super(ctx, 0, trxName);
		System.out.println("Creando por Fact_Line org.compiere.acct");	
	...
	{
        /*
		// begin Victor P̩rez e-evolution 30.08.2005
		// fix Debit & Credit 
		if (AmtSourceDr != null)
		{	
		...
		*/
        ...
        /**
         * Fix to consider when the fact have both, Dr and Cr
         * 
         * @author Ezequiel Scott @ Zynnia
         */
        if ( AmtSourceDr == null || AmtSourceCr == null ){
            if (AmtSourceDr != null)
            {	
                if (AmtSourceDr.compareTo(Env.ZERO) == -1)
                {	
	...
	/**
	 *  Convert to Accounted Currency with rate
	 *  @author Daniel 21/07/2009
	 *  @return true if converted
	 */
	public boolean convert (BigDecimal rate)
	{
		...
		MCurrency moneda = MCurrency.get (Env.getCtx(), m_acctSchema.getC_Currency_ID());
                setAmtAcctDr ( (getAmtSourceDr().multiply(rate)).setScale(moneda.getStdPrecision(),BigDecimal.ROUND_HALF_UP));
		setAmtAcctCr ( (getAmtSourceCr().multiply(rate)).setScale(moneda.getStdPrecision(),BigDecimal.ROUND_HALF_UP));
		return true;
	}	//	convert
	...
	*
	*	Creado para saber si un registro contable es DEBE o HABER.
    */
