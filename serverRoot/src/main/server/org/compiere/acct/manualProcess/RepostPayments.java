...
public class RepostPayments {
    ...
    private static  String sql = "select p.* from c_valorpago  vp "
    ...
    public static void main(String[] args) {
         ...
         String sql = "select p.C_Payment_ID from c_valorpago  vp "
                                   + " JOIN c_payment p on p.c_payment_id = vp.c_payment_id "
                                   + " WHERE vp.nrocheque in ( ";
        
        int initCheckNumber = 78044909;
        int numberOfChecks = 74;
        for (int i = 0 ; i < numberOfChecks ; i++) {
            sql += "'"+(initCheckNumber + i)+"',";
        }
