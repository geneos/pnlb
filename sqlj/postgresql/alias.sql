...
CREATE OR REPLACE FUNCTION addDays (day INTERVAL , days DECIMAL)
RETURNS TIMESTAMP WITH TIME ZONE AS $$
BEGIN
    RETURN addDays(CAST (day AS TIMESTAMP WITH TIME ZONE),ID(days));
END;
...
CREATE OR REPLACE FUNCTION compiere.bomQtyAvailable(M_Product_ID NUMERIC, M_AttributeSetInstance_ID NUMERIC,M_Warehouse_ID NUMERIC, M_Locator_ID NUMERIC)
  RETURNS NUMERIC AS
$BODY$
BEGIN
    RETURN bomQtyAvailable(ID(M_Product_ID), ID(M_AttributeSetInstance_ID),ID(M_Warehouse_ID),ID(M_Locator_ID));
END;    
$BODY$
LANGUAGE plpgsql VOLATILE;
--ALTER FUNCTION compiere.bomqtyavailable("numeric", "numeric", "numeric", "numeric") OWNER TO compiere;

CREATE OR REPLACE FUNCTION compiere.bomQtyOnHand(M_Product_ID NUMERIC, M_AttributeSetInstance_ID NUMERIC,M_Warehouse_ID NUMERIC, M_Locator_ID NUMERIC)
  RETURNS NUMERIC AS
$BODY$
BEGIN
...
CREATE OR REPLACE FUNCTION compiere.bomQtyOrdered(M_Product_ID NUMERIC, M_AttributeSetInstance_ID NUMERIC,M_Warehouse_ID NUMERIC, M_Locator_ID NUMERIC)
  RETURNS NUMERIC AS
$BODY$
BEGIN
    RETURN bomQtyOrdered(ID(M_Product_ID), ID(M_AttributeSetInstance_ID),ID(M_Warehouse_ID),ID(M_Locator_ID));
END;    
$BODY$
LANGUAGE plpgsql VOLATILE;
--ALTER FUNCTION compiere.bomqtyordered("numeric", "numeric", "numeric", "numeric") OWNER TO compiere;

CREATE OR REPLACE FUNCTION compiere.bomQtyReserved(M_Product_ID NUMERIC, M_AttributeSetInstance_ID INTEGER,M_Warehouse_ID NUMERIC, M_Locator_ID NUMERIC)
  RETURNS NUMERIC AS
$BODY$
BEGIN
    RETURN bomQtyReserved(ID(M_Product_ID), ID(M_AttributeSetInstance_ID),ID(M_Warehouse_ID),ID(M_Locator_ID));
END;    
$BODY$
LANGUAGE plpgsql VOLATILE;
...
CREATE OR REPLACE FUNCTION documentNo (p_MPC_MRP_ID NUMERIC)
RETURNS VARCHAR AS $$
BEGIN
    RETURN documentNo(ID(p_MPC_MRP_ID));
END;    
$$ LANGUAGE plpgsql VOLATILE;	

DROP OPERATOR compiere.+ (timestamptz, INTEGER);
CREATE OPERATOR compiere.+ ( PROCEDURE = compiere.adddays,
LEFTARG = TIMESTAMPTZ, RIGHTARG = INTEGER,
COMMUTATOR = +);

DROP OPERATOR compiere.- (timestamptz, INTEGER);
CREATE OPERATOR compiere.- ( PROCEDURE = compiere.subtractdays,
LEFTARG = TIMESTAMPTZ, RIGHTARG = INTEGER,
COMMUTATOR = -);


DROP OPERATOR compiere.+ (interval, numeric);
CREATE OPERATOR compiere.+ ( PROCEDURE = compiere.adddays,
