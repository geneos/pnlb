...
public class MMPCOrder extends X_MPC_Order implements DocAction {
    ...
    public String toString() {
        StringBuffer sb = new StringBuffer("MMPCOrder[")
			...
			if (product != null)
			{
				if (product.isStocked())
				{
					...
					int M_Locator_ID = 0;
                                        ...
                                        //Obtengo partidas segun FeFo para el listado (No tengo en cuenta partidas vencidas)
                                        MStorage[] storages = MStorage.getAllWithASIFEFO(getCtx(), line.getM_Product_ID()/*,line.getM_Warehouse_ID()*/, true, false, get_TrxName());
					BigDecimal orden = BigDecimal.ZERO;
                                        //Reservo segun FeFo
                                        for (int s=0; s<storages.length && reserved.compareTo(BigDecimal.ZERO) > 0 ; s++){
                                            if (reserved.compareTo(BigDecimal.ZERO) == -1){
                                                log.severe("Error al reservar cantidades para producto: "+line.getM_Product_ID());
                                                return false;
                                            }
                                            MStorage aStorage = storages[s];
                                            BigDecimal available = aStorage.getQtyOnHand().subtract(aStorage.getQtyReserved());
                                            BigDecimal toReserve = BigDecimal.ZERO;
                                            
                                            //Me fijo si lo que necesito reservar supera a lo disponible
                                             ...
                                             * GENEOS - Pablo Velazquez
                                             * 07/08/2013
                                             * Fix para que si lo que voy a reservar es 0, no lo reserve 
                                             * toReserve puede ser 0, cuando la cantidad disponible (available) es 0
                                             */
                                            if (toReserve.compareTo(BigDecimal.ZERO) == 0)
                                                continue;
                                            
                                            if (toReserve.compareTo(BigDecimal.ZERO) == -1){
                                                log.severe("Error al reservar cantidades para producto: "+line.getM_Product_ID()+"cantidad a reservar negativa");
                                                return false;
                                            }
                                            
                                            if (!MStorage.addDist(getCtx(), line.getM_Warehouse_ID(), aStorage.getM_Locator_ID(),
						line.getM_Product_ID(),
						aStorage.getM_AttributeSetInstance_ID(), aStorage.getM_AttributeSetInstance_ID(),
						Env.ZERO, toReserve, Env.ZERO, get_TrxName()))
						return false;
                                            /*
                                             * GENEOS - Pablo Velazquez
                                             * 27/07/2013
                                             * Ademas de reservarse el stock en cada partida especifica, tambien
                                             * se guarda que cantidad de reservados pertenece a cada orden de manufactura
                                             * y cuanto queda disponible de lo reservado.
                                             * para poder cotrolar varios aspectos en otras instancias.
                                             *  - Descontar de forma apropiada los reservados cuando surto mas de lo que necesito
                                             *  - Cuando se surte manualmente saber las cantidades disponibles reales
                                             */
                                            MMPCOrderQtyReserved orderRes = MMPCOrderQtyReserved.getCreate(getCtx(), aStorage, line.getMPC_Order_BOMLine_ID(), get_TrxName());
                                            orderRes.setTotalQty(toReserve);
                                            orderRes.setRemainingQty(toReserve);
                                            
                                            orderRes.setUseOrder(orden);
                                            orden = orden.add(new BigDecimal(1));
                                            if (!orderRes.save())
                                                return false;                                 
                                            reserved = reserved.subtract(toReserve);
                                        }
                                        /*
                                         * Si el producto no es critico y faltan partidas disponibles para reservar
                                         * Entonces reservo en storage CERO y creo un control de reservados, para que el mismo
                                         * se actualize de forma correcta al surtir
                                         */
                                        if (reserved.compareTo(BigDecimal.ZERO) == 1)
                                            if ( !line.isCritical() ) {
						...
						M_Locator_ID = wh.getDefaultLocator().getM_Locator_ID();
                                                ...
                                                MMPCOrderQtyReserved orderRes = MMPCOrderQtyReserved.getCreate(getCtx(), storageDefaultZero, line.getMPC_Order_BOMLine_ID(), get_TrxName());
                                                orderRes.setTotalQty(reserved);
                                                orderRes.setRemainingQty(reserved);
                                                orderRes.setUseOrder(orden);
                                                if (!orderRes.save())
                                                    return false;  
                                            }
                                            else {
                                                log.severe("Error al reservar cantidades para producto: "+line.getM_Product_ID()+ " faltan: "+reserved + " disponibles para reservar");
                                                return false;
                                            }
				}	//	stockec
				//	update line
			...
			if (product != null)
			{
				if (product.isStocked())
				{
					...
					BigDecimal reserved = difference;
                                         ...
                                         * GENEOS - Pablo Velazquez
                                         * 06/08/2013
                                         * Modificacion para que al desreservar, lo haga de las partidas especificas correspondientes
                                         * que se indican en el control MPC_ORDER_QTYRESERVED
                                         */
                                        
                                        //Obtengo todas las reservas para la linea
                                        MMPCOrderQtyReserved qtyRes[] = MMPCOrderQtyReserved.getAllForBOMLine(getCtx(), line.getMPC_Order_BOMLine_ID(), get_TrxName());
					
                                        //Por cada reserva
                                        for (int r=0; r<qtyRes.length; r++){
                                            MMPCOrderQtyReserved qtyResCtrl = qtyRes[r];
                                            
                                            BigDecimal toUnreserve = Env.ZERO.subtract(qtyResCtrl.getRemainingQty());
                                            
                                            //	Actualizo Storage
                                            if (!MStorage.addDist(getCtx(), line.getM_Warehouse_ID(), qtyResCtrl.getM_Locator_ID(),
						line.getM_Product_ID(),
						qtyResCtrl.getM_AttributeSetInstance_ID(), qtyResCtrl.getM_AttributeSetInstance_ID(),
						Env.ZERO, toUnreserve, Env.ZERO, get_TrxName()))
						return false;
                                            
                                            //Actualizo Control
                                            qtyResCtrl.setTotalQty(qtyResCtrl.getTotalQty().add(toUnreserve));
                                            qtyResCtrl.setRemainingQty(Env.ZERO);
                                            if (!qtyResCtrl.save())
                                                return false;
                                        }
                                        /*
                                         * Si no tiene control de reservados entonces saco el reservado de el storage CERO
                                         */
                                        if (qtyRes.length == 0){            
                                            MWarehouse wh = MWarehouse.get(getCtx(), line.getM_Warehouse_ID());
    ...
    private BigDecimal getMaterialsCost(int MPC_Order_ID,int MPC_Cost_Element_ID,int C_AcctSchema_ID, boolean generarDetalleCosto){
        ...
        for (int i=0;i<ra.length;i++){
            ...
            if (doc.getDocBaseType().equalsIgnoreCase(MDocType.DOCBASETYPE_ManufacturingOrderIssue)
                ...
                if (ra[i].getM_Product_ID()!=this.getM_Product_ID())
                    //si es materia prima...
...
//                            MPC_Order_Cost.setM_CostElement_ID(cost[j].getM_CostElement_ID());
    ...
    public boolean deleteReserveCtrl(MMPCOrderBOMLine[] lines){
        
        for (int i=0; i<lines.length; i++) {
            //Obtengo todas las reservas para la linea
            MMPCOrderQtyReserved qtyRes[] = MMPCOrderQtyReserved.getAllForBOMLine(getCtx(), lines[i].getMPC_Order_BOMLine_ID(), get_TrxName());
            //Por cada reserva la elimino
            for (int r=0; r<qtyRes.length; r++){
                if ( !qtyRes[r].delete(true) ){
                    return false;
                }
            }
        }
    ...
    public MMPCOrderQtyReserved[] getReserveCtrlLines(){
    ...
    // fjviejo e-evolution complete mpcorder cuando checa si hay cantidad disponible que no lo separe por atributo begin
     public boolean isAvailable_old()
	...
	public boolean closeIt()
	{
            ...
            for (int i=0 ; i<lines.length ; i++){
                ...
                if (pct != null){
                    ...
                    BigDecimal maxQtyTolerance = lines[i].getQtyRequiered().add(lines[i].getQtyRequiered().multiply(tolerance));
                    BigDecimal minQtyTolerance = lines[i].getQtyRequiered().subtract(lines[i].getQtyRequiered().multiply(tolerance));
                    //Sobrepasa limite superior?
                    if ( lines[i].getQtyDelivered().compareTo(maxQtyTolerance) == 1 ){
                        toleranceAccomplishedMayor = false;
                        productsMayores += producto.getValue() + " - " + producto.getName()+"\n";
                    }
                    
                    //No llega a cubrir limite inferior?
                    if ( lines[i].getQtyDelivered().compareTo(minQtyTolerance) == -1 ){
                        toleranceAccomplishedMenor = false;
                        productsMenores += producto.getValue() + " - " + producto.getName()+"\n";
                    }
                }
            }
