...
public class MInOut extends org.compiere.model.MInOut implements DocAction {
    ...
    protected boolean beforeSave(boolean newRecord) {
        ...
        if (doc.getDocBaseType().equals(MDocType.DOCBASETYPE_RecepcionMaterialesTerceros)
            ...
            /*
             * Verifico obligatoriedad de campos para devolucion de producto terminado
             * Nro de Bultos: Se utiliza el campo descripción
             * Numero de remito: Se utiliza de campo "Referencia de Orden de Socio"
             * Fecha de Remito: Se utiliza el campo "Fecha de la Orden
             */
            if (isReturn()) {
                boolean returnError = false;
                String msgErrorReturn = "";
                
                if (Util.isEmpty(getDescription())) {
                    msgErrorReturn+= "Por favor ingrese el campo Nro de Bultos (Descripcion)\n";
                    returnError = true;
                }
                if (Util.isEmpty(getPOReference() )) {
                    msgErrorReturn+= "Por favor ingrese el campo Nro de Remito (Referencia de Orden de Socio)\n";
                    returnError = true;
                }
                if (getDateOrdered() == null) {
                    msgErrorReturn+= "Por favor ingrese el campo Fecha de Remito (Fecha de la orden)\n";
                    returnError = true;
                }
                if (returnError){
                    JOptionPane.showMessageDialog(null, msgErrorReturn, "Campos obligatorios devolucion de terminado", JOptionPane.ERROR_MESSAGE);
                    return false;
                }
            }
            /**
