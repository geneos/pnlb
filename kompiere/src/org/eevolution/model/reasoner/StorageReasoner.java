...
public class StorageReasoner {
	...
	public MMPCOrderWorkflow getMPCOrderWorkflow(MMPCOrder order) {
		
		int[] ids = PO.getAllIDs("MPC_Order_Workflow", "MPC_Order_ID = "+order.get_ID(), null);
		if(ids.length != 1) {
			
			return null;
		}
		
		return new MMPCOrderWorkflow(Env.getCtx(), ids[0], null);
	}
