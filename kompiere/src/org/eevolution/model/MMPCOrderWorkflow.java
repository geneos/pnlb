...
public class MMPCOrderWorkflow extends X_MPC_Order_Workflow
{
	...
	public static MMPCOrderWorkflow getDocValue (Properties ctx, int AD_Client_ID, int AD_Table_ID)
	{
		...
		{
			String sql = "SELECT * FROM MPC_Order_Workflow "
	...
	{
		String sql = "SELECT * FROM MPC_Order_Node WHERE MPC_Order_Workflow_ID=? AND IsActive='Y' AND AD_Client_ID = ?" ;
		try
		{
		...
		{
			log.log(Level.SEVERE, "MMPCOrderWorkflow.loadNodes", e);
		}
	...
	private MMPCOrderNode[] getNodesInOrder()
	{
		if (m_nodeArray == null)
		{
			...
			if (m_nodes.size() != list.size()){
				System.out.println("MMPCOrderWorkflow.getNodesInOrder - sizes different: " + m_nodes.size() + "<->" + list.size());
                //log.log(Level.SEVERE, "MMPCOrderWorkflow.getNodesInOrder - sizes different: " + m_nodes.size() + "<->" + list.size());
                MMPCOrder order = new MMPCOrder(getCtx(), getMPC_Order_ID(), get_TrxName());
                log.warning("Las transisiones de los nodos están mal configuradas para la orden: nº "+order.getDocumentNo());
            }
			//
