            ...
            if (rs.next()) {
                ...
                if (IsSOTrx) {
                    ...
                    if (CreditLimit != 0) {
                        ...
                        if (!rs.wasNull() && CreditAvailable < 0) //mTab.fireDataStatusEEvent("CreditLimitOver",DisplayType.getNumberFormat(DisplayType.Amount).format(CreditAvailable));                                                                                                          
                        {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                if (IsSOTrx) {
                    ...
                    if (CreditLimit != 0) {
                        double CreditAvailable = rs.getDouble("CreditAvailable");
                        if (!rs.wasNull() && CreditAvailable < 0) //mTab.fireDataStatusEEvent("CreditLimitOver",isplayType.getNumberFormat(DisplayType.Amount).format(CreditAvailable));
                        {
                            JOptionPane.showMessageDialog(null, "CreditLimitOver", DisplayType.getNumberFormat(DisplayType.Amount).format(CreditAvailable), JOptionPane.ERROR_MESSAGE);
                        }
                    }
...
//	}	//	product
    ...
    public String tax(Properties ctx, int WindowNo, MTab mTab, MField mField, Object value) {
        ...
        if (steps) {
            log.log(Level.WARNING, "tax - init");
        }
        ...
        //
        if (C_Tax_ID == 0) //mTab.fireDataStatusEEvent(log.retrieveError());
        {
        ...
        //
        if (steps) {
            log.log(Level.WARNING, "tax - fini");
        }
        return amt(ctx, WindowNo, mTab, mField, value);
    }	//	tax
...
//		int M_Product_ID = Env.getContextAsInt(ctx, WindowNo, "M_Product_ID");
        // int MPC_ProfileBOM_ID = Env.getContextAsInt(Env.getCtx(), WindowNo, "#MPC_PrifileBOM_ID");
        ...
        Qty = Env.ZERO;
        //
        Integer MPC_ProfileBOM_ID = (Integer) mTab.getValue("MPC_ProfileBOM_ID");
        ...
        System.out.println("profilebom------------ " + MPC_ProfileBOM_ID);
        try {
            StringBuffer plv = new StringBuffer("SELECT PriceList,Qty FROM MPC_ProfileBOM WHERE IsActive='Y' AND AD_Client_ID=? and MPC_ProfileBOM_ID=? ");
            PreparedStatement pstmtplv = DB.prepareStatement(plv.toString());
            pstmtplv.setInt(1, AD_Client_ID);
            pstmtplv.setInt(2, MPC_ProfileBOM_ID.intValue());
     ...
     *  @param value    The new value
     */
    public String margin(Properties ctx, int WindowNo, MTab mTab, MField mField, Object value) {
        if (isCalloutActive() || value == null) {
            return "";
        }
        setCalloutActive(true);
...
//				Env.setContext(ctx, WindowNo, "M_PriceList_Version_ID", rs.getInt(5));
        ...
        StringBuffer SQL1 = new StringBuffer("SELECT Value From MPC_ProfileBOM where IsActive='Y' and Value LIKE '");
...
//				Env.setContext(ctx, WindowNo, "M_PriceList_Version_ID", rs.getInt(5));
    ...
    public String productEprice(Properties ctx, int WindowNo, MTab mTab, MField mField, Object value) {
        Integer M_ProductE_ID = (Integer) value;
        if (M_ProductE_ID == null || M_ProductE_ID.intValue() == 0) {
            return "";
        }
        if (steps) {
            log.log(Level.WARNING, "product - init");
        }
        setCalloutActive(true);
...
//			mTab.setValue("M_AttributeSetInstance_ID", null);
        ...
        int AD_Client_ID = Integer.parseInt(Env.getContext(Env.getCtx(), "#AD_Client_ID"));
        try {
            StringBuffer plv = new StringBuffer("SELECT p.ShelfWidth FROM MPC_ProfileBOM mp INNER JOIN M_Product p ON(p.M_Product_ID=mp.M_Product_ID) WHERE p.IsActive='Y' AND p.AD_Client_ID=? and mp.MPC_ProfileBOM_ID=? ");
            PreparedStatement pstmtplv = DB.prepareStatement(plv.toString());
            pstmtplv.setInt(1, AD_Client_ID);
            pstmtplv.setInt(2, MPC_ProfileBOM_ID.intValue());
            //pstmt.setInt(2, m_M_PriceList_ID);
        ...
        setCalloutActive(false);
        if (steps) {
            log.log(Level.WARNING, "product - fini");
        }
        return amt(ctx, WindowNo, mTab, mField, value);
    }	//	product

    public String productEtprice(Properties ctx, int WindowNo, MTab mTab, MField mField, Object value) {
...
//			mTab.setValue("M_AttributeSetInstance_ID", null);
    ...
    public String productTprice(Properties ctx, int WindowNo, MTab mTab, MField mField, Object value) {
...
//			mTab.setValue("M_AttributeSetInstance_ID", null);
        ...
        int AD_Client_ID = Integer.parseInt(Env.getContext(Env.getCtx(), "#AD_Client_ID"));
        try {
            StringBuffer plv = new StringBuffer("SELECT mp.Qty FROM MPC_ProfileBOM mp WHERE mp.AD_Client_ID=? and mp.MPC_ProfileBOM_ID=? ");
            PreparedStatement pstmtplv = DB.prepareStatement(plv.toString());
            pstmtplv.setInt(1, AD_Client_ID);
            pstmtplv.setInt(2, MPC_ProfileBOM_ID.intValue());
            //pstmt.setInt(2, m_M_PriceList_ID);
