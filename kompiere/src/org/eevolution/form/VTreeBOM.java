...
import org.compiere.model.X_MPC_Product_BOM;
import org.compiere.model.X_MPC_Product_BOMLine;
...
public class VTreeBOM extends CPanel
    ...
    private void action_loadBOM() {
        ...
        splitPane.add(new JScrollPane(m_tree), JSplitPane.LEFT);
    ...
    public DefaultMutableTreeNode parent(X_MPC_Product_BOM bom) {
        ...
        String data = Msg.translate(Env.getCtx(), "MPC_Product_BOM_ID") + " " + Msg.translate(Env.getCtx(), "Value") + ":" + bom.getValue() + " " + Msg.translate(Env.getCtx(), "Name") + ": " + bom.getName();
        DefaultMutableTreeNode parent = new DefaultMutableTreeNode(data);

        QueryDB query = new QueryDB("org.compiere.model.X_MPC_Product_BOMLine");
        String filter = "MPC_Product_BOM_ID=" + bom.getMPC_Product_BOM_ID()+" AND isActive='Y'";
        java.util.List results = query.execute(filter);
        Iterator select = results.iterator();
        while (select.hasNext()) {
            X_MPC_Product_BOMLine bomline = (X_MPC_Product_BOMLine) select.next();
