 ...
 */
public class GenerateReporteCtaCteMonedaOriginalG extends SvrProcess{
...
protected String doIt() throws Exception {
    ...
    if (rs.next())
    {
	    ...
	    UtilProcess.initViewer("Cuenta Corriente en Moneda Original por Grupo",0,getProcessInfo());
	    return "";
    } 
    ...
    private BigDecimal obtenerSaldoInicial(int BPartner_ID, int BPartner_Location_ID) throws Exception
    {
        ...
        {	sql = 
	        ...
	        "FROM ( " +
				...
				" WHERE i.DocStatus IN ('CO','CL') AND i.ISSOTRX=? AND b.C_BPARTNER_ID = ? AND i.C_CURRENCY_ID = ? AND i.DATEINVOICED<? AND bpl.C_BPartner_Location_ID = ? " +
				"UNION " +
				"( " +
				" SELECT " +
	            " 	CASE d.docbasetype " +
				...
				" INNER JOIN C_Location l ON (i.BILL_LOCATION_ID = l.C_Location_ID) " +
				" INNER JOIN C_BPartner_Location bpl ON (bpl.C_LOCATION_ID = l.C_Location_ID) " +
				" WHERE C_Payment.DocStatus IN ('CO','CL') AND C_Payment.ISRECEIPT=? AND b.C_BPARTNER_ID = ? AND C_Payment.C_CURRENCY_ID = ? AND C_Payment.DATETRX<? AND bpl.C_BPartner_Location_ID = ? " +
				") )";
        ...
        else
        {
	        ...
	        "FROM ( " +
				...
				" INNER JOIN C_BPARTNER b ON(i.C_BPARTNER_ID = b.C_BPARTNER_ID) " +
				" INNER JOIN C_DocType d ON(i.C_DocType_ID = d.C_DocType_ID) " + 
				" WHERE i.DocStatus IN ('CO','CL') AND i.ISSOTRX=? AND b.C_BPARTNER_ID = ? AND i.C_CURRENCY_ID = ?" +
				...
				" INNER JOIN C_AllocationLine al ON (al.C_Payment_ID = C_Payment.C_Payment_ID)" +
				" INNER JOIN C_Invoice i ON (al.C_Invoice_ID = i.C_Invoice_ID)" +
				" WHERE C_Payment.DocStatus IN ('CO','CL') AND C_Payment.ISRECEIPT=? AND b.C_BPARTNER_ID = ? AND C_Payment.C_CURRENCY_ID = ?" +
            ...
            pstmt.setInt(11, C_Currency_ID.intValue());
