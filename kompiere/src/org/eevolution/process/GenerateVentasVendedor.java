 ...
 *  Date 20/09/2009
 */
public class GenerateVentasVendedor extends SvrProcess{
    ...
    protected String doIt() {
        try {
            ...
            UtilProcess.initViewer("Ventas por Vendedor",p_PInstance_ID,getProcessInfo());
        } catch (SQLException ex) {
            Logger.getLogger(GenerateVentasVendedor.class.getName()).log(Level.SEVERE, null, ex);
        }
	...
	private ResultSet getProductos()		throws Exception{
		...
		String sql =
			...
			" FROM ( " +
			"    SELECT i.AD_CLIENT_ID, i.AD_ORG_ID, i.SALESREP_ID as ID, u.NAME, " +
			...
			getSqlWhere("i.ISSOTRX","i.DATEINVOICED","i.DocStatus","bp.IsSalesRep") +
			"    GROUP BY i.AD_CLIENT_ID, i.AD_ORG_ID, i.SALESREP_ID, u.NAME, d.DOCBASETYPE " +
			" ) " +
    ...
    {
    	log.info("Borrado de la tabla temporal T_VentasVendedor");
    	DB.executeUpdate("Delete from T_VentasVendedor", null);
    	log.info("Borrado de la tabla temporal T_VentasVendedor_HDR");
	    DB.executeUpdate("Delete from T_VentasVendedor_HDR", null);
	    
    	String sql = "";
    	try {
			ResultSet rs = getProductos();
			
		    int T_VentasVendedor_ID=1000000;
		    ...
		    if (rs.next())
		    {
		    	...
		    	sql = "INSERT INTO T_VentasVendedor_HDR VALUES(?,?,'Y',?,?,?,?)";
		    	...
		    	while (!rs.isAfterLast()){
			    	...
			    	sql = "INSERT INTO T_VentasVendedor VALUES(?,?,'Y',?,?,?,?,?,?,?)";
			    	ps = DB.prepareStatement(sql, null);
			        ps.setInt(1, getAD_Client_ID());			//	CLIENTE
			        ps.setInt(2, rs.getInt(2));					//	ORGANIZACION
			        ps.setInt(3, T_VentasVendedor_ID);			//	C_VENTASVENDEDOR_ID
			        ps.setInt(4, p_PInstance_ID);				//	INSTANCE
			        ps.setInt(5, rs.getInt(3));					//	AD_USER_ID
			        ps.setString(6, rs.getString(4));			//	NAME USER
		        ...
		        ps.setInt(3, T_VentasVendedor_ID);			//	C_VENTASVENDEDOR_ID
		        ps.setInt(4, p_PInstance_ID);				//	INSTANCE
		        ps.setInt(5, getAD_User_ID());				//	AD_USER_ID
		        ps.setString(6, "TOTAL");					//	NAME USER
