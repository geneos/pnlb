 ...
 *	
 *  @author Jorg Janke
 *  @version $Id: RequisitionPOCreate.java,v 1.4 2006/01/21 02:23:37 jjanke Exp $
 */
public class RequisitionPOCreate extends SvrProcess
{
	...
	/** Order				*/
	private org.eevolution.model.MOrder		m_order = null;
	/** Order Line			*/
	private org.eevolution.model.MOrderLine	m_orderLine = null;
	...
	protected String doIt() throws Exception
	{
		...
		if (p_M_Requisition_ID != 0)
		{
			...
			org.eevolution.model.MRequisition req = new org.eevolution.model.MRequisition(getCtx(), p_M_Requisition_ID, get_TrxName());
			if (!org.eevolution.model.MRequisition.DOCSTATUS_Completed.equals(req.getDocStatus()))
				throw new CompiereUserError("@DocStatus@ = " + req.getDocStatus());
			org.eevolution.model.MRequisitionLine[] lines = req.getLines();
		...
		try
		{
			...
			while (rs.next ())
			{
				process (new org.eevolution.model.MRequisitionLine (getCtx(), rs, get_TrxName()));
			}
			rs.close ();
			pstmt.close ();
			pstmt = null;
	 ...
	 */
	private void process (org.eevolution.model.MRequisitionLine rLine) throws Exception
	{
	...
	private void newOrder(MRequisitionLine rLine, int C_BPartner_ID) throws Exception
	{
		...
		//	Order
		m_order = new org.eevolution.model.MOrder(getCtx(), 0, get_TrxName());
	...
	private void newLine(MRequisitionLine rLine) throws Exception
	{
		...
		else
		{
			...
			if (C_BPartner_ID == 0)
				throw new CompiereUserError("No Vendor for " + product.getName());
		}
		...
		//	No Order Line
		m_orderLine = new org.eevolution.model.MOrderLine(m_order);
		if (product != null)
		{
		...
		//begin vpj-cd e-evolution 26 abril
                org.compiere.model.MRequisition r = rLine.getParent();
                m_orderLine.setDateOrdered(r.getDateDoc());
                m_orderLine.setDatePromised(r.getDateRequired());
                //m_orderLine.setDatePromised(rLine.)
                //end
