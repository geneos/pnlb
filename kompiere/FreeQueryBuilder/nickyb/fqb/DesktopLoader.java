		...
		throws SQLException
	{
		...
		if(desktop.builder.getConnection()!=null)
		{
			...
			while(rsColumns.next())
			{
				...
				org.compiere.model.M_Element element = org.compiere.model.M_Element.get(org.compiere.util.Env.getCtx(),columnName);
				//DesktopEntity.Field field = item.addField(columnName, primary.containsKey(columnName),size,infos);
				DesktopEntity.Field field = item.addField(columnName, org.compiere.util.Msg.getMsg(Env.getCtx(),element.getName()), primary.containsKey(columnName),size,infos);
