...
public class QueryBrowser extends DefaultPanel
{
	...
	private JTree tree;
	//begin e-evolution 05/16/2005
	private String tablename = null;
	private String table = null;
	//end e-evolution 05/16/2005
	...
	{
		tree = new JTree(new DefaultTreeModel(new DefaultMutableTreeNode("<no connection>")));
	...
	void connectionChanged() throws SQLException
	{
		...
		DefaultMutableTreeNode root = new DefaultMutableTreeNode("<no connection>");
		...
		if(builder.getConnection()!=null)
		{
			...
			if(builder.jdbcUseSchema())
			{
				...
				while(rsSchemas.next())
				{	
					if (rsSchemas.getString(1).toLowerCase().equals("compiere"))
					root.add(new DefaultMutableTreeNode(rsSchemas.getString(1), true));
				}	
		...
		//root.add(new DefaultMutableTreeNode("COMPIERE", true));
		//end vpj-cd e-evolution 05/16/2005
	...
	private void loadTables(DefaultMutableTreeNode parent)
	{
		...
		try
		{
			...
			ResultSet rsTables = dbmd.getTables(catalog, schema, "%", new String[]{"TABLE","VIEW"});
			...
			if(rsTables!=null)
			{
				...
				while(rsTables.next())
				{
					...
					String sql = "SELECT AD_Table.TableName, AD_Table.Name FROM AD_Table INNER JOIN AD_WINDOW ON AD_TABLE.AD_WINDOW_ID = AD_WINDOW.AD_WINDOW_ID INNER JOIN AD_WINDOW_ACCESS ON AD_WINDOW.AD_WINDOW_ID = AD_WINDOW_ACCESS.AD_WINDOW_ID INNER JOIN AD_ROLE ON AD_WINDOW_ACCESS.AD_ROLE_ID = AD_ROLE.AD_ROLE_ID WHERE UPPER(TableName)=UPPER('" +rsTables.getString(3) +"') ";
			...
			//Arrays.sort(sortcols,String.CASE_INSENSITIVE_ORDER);
		...
		public void mousePressed(MouseEvent e)
		{
			...
			if(this.isTable(path))
			{
				...
				try
				{
					...
					String sql = "SELECT TableName FROM AD_Table WHERE LOWER(Name)=LOWER('" +name +"')";
	...
	private class ActionRefresh extends AbstractAction
	{
		ActionRefresh()
		{
			...
			this.putValue(SHORT_DESCRIPTION, "refresh");
		}
