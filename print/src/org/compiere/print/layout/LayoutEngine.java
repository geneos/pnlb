 ...
 *
 * @author Jorg Janke
 * @version $Id: LayoutEngine.java,v 1.73 2005/11/13 23:40:21 jjanke Exp $
 */
public class LayoutEngine implements Pageable, Printable, Doc {
     ...
     */
    private int m_headerHeight = 24;		//	1/4" => 72/4
    /**
     * Footer Area Height (1/4")
     */
    private int m_footerHeight = 24;
    /**
     * Current Page Number
     */
    ...
    public void setPaper(CPaper paper, int headerHeight, int footerHeight) {
        ...
        //
        log.log(Level.FINE, "{0} - Header={1}, Footer={2}", new Object[]{paper, headerHeight, footerHeight});
        m_paper = paper;
    ...
    private void calculatePageSize() {
        ...
        log.log(Level.FINE, "Paper={0},HeaderHeight={1},FooterHeight={2} => Header={3},Contents={4},Footer={5}", new Object[]{m_paper, m_headerHeight, m_footerHeight, m_header, m_content, m_footer});
//        System.out.println("Paper=" + m_paper + ",HeaderHeight=" + m_headerHeight + ",FooterHeight=" + m_footerHeight + " => Header=" + m_header + ",Contents=" + m_content + ",Footer=" + m_footer);
    }	//	calculatePageSize
    ...
    private void verificarTransport(PrintElement element) {

        if (((TableElement) element).getTansportLayout().isActive()) {
            ((TableElement) element).desplazar();
            // ((TableElement)element).calculateSize();
        }
    ...
    private void layout() {
        ...
        /*
         * Modificacion realizada para cambiar la vizualicion del nro de pagina en los reportes tipo libro
         * Fecha:06/02/2008 Autor:Avendaío Nadia
         */
        if (this.getFormat() != null && this.getFormat().getCtx().get("typePrint") != null && ((String) (this.getFormat().getCtx().get("typePrint"))).equals("LIBRO")) {
            m_pageNo = ((Long) (this.getFormat().getCtx().get("startPage"))).intValue() - 1;
        } else {
            m_pageNo = 0;
        }
        //fin modificacion
        ...
        //	initialize
		/*
         * Modificacion realizada por Mario Giacomaso para que imprima o no los parametros seg�n el valor chequeado en
         * la configuracion del print format.
         */
        //	Parameter
         ...
         */
        /*
         * String pageInfo = String.valueOf(m_pages.size()) + getPageInfo(m_pages.size()); Env.setContext(m_printCtx,
         * Page.CONTEXT_PAGECOUNT, pageInfo);
         */
        ...
        for (int i = 0; i < pages; i++) {
            ...
            if (this.getFormat() != null && this.getFormat().getCtx().get("typePrint") == null) {
                String pageInfo = page.getPageInfo() + "/" + String.valueOf(pages); //+ getPageInfo(pageNo);
         ...
         */
        if (this.getFormat() != null && this.getFormat().getCtx().get("typePrint") != null) {
            this.getFormat().getCtx().remove("typePrint");

        }
        if (this.getFormat() != null && this.getFormat().getCtx().get("startPage") != null) {
            this.getFormat().getCtx().remove("startPage");
        }
    }	//	layout
    ...
    protected int newPage(boolean force, boolean preserveXPos) {
        ...
        log.log(Level.FINER, "Page={0}", m_pageNo);
        return m_pageNo;
    }	//	newPage
    ...
    protected void newLine() {
        ...
        if (isYspaceFor(m_maxHeightSinceNewLine[m_area])) {
            ...
            log.log(Level.FINEST, "Page={0} [{1}] {2}/{3}", new Object[]{m_pageNo, m_area, m_position[m_area].x, m_position[m_area].y});
        } else if (m_area == AREA_CONTENT) {
            log.log(Level.FINEST, "Not enough Y space {0} - remaining {1} - Area={2}", new Object[]{m_lastHeight[m_area], getYspace(), m_area});
            newPage(true, false);
            log.log(Level.FINEST, "Page={0} [{1}] {2}/{3}", new Object[]{m_pageNo, m_area, m_position[m_area].x, m_position[m_area].y});
        } else //	footer/header
        {
            m_position[m_area].setLocation(part.x, m_position[m_area].y + m_maxHeightSinceNewLine[m_area]);
            log.log(Level.SEVERE, "Outside of Area({0}): {1})", new Object[]{m_area, m_position[m_area]});
        }
    ...
    public Page getPage(int pageNo) {
        if (pageNo <= 0 || pageNo > m_pages.size()) {
            log.log(Level.SEVERE, "No page #{0}", pageNo);
            return null;
        }
    ...
    public LayoutEngine getPageable(boolean isCopy) {
        ...
        if (getNumberOfPages() == 0
            ...
            log.log(Level.WARNING, "Nothing to print - {0}", toString());
            return null;
        }
        return this;
    }	//	getPageable
    ...
    protected void addY(int yOffset) {
        ...
        if (isYspaceFor(yOffset)) {
            ...
            m_position[m_area].y += yOffset;
            log.log(Level.SEVERE, "Outside of Area: {0}", m_position);
        }
    }	//	addY
    ...
    private void createStandardHeaderFooter() {
        ...
        //to change the header
                /*
         * Modificacion realizada para cambiar la vizualizacion del nro de pagina en los libros de IVA Fecha:06/02/2008
         * Autor: Avenda Nadia
         */
            ...
            for (int i = 0; i < m_format.getItemCount(); i++) {
                ...
                if (item.isRelativePosition() && !lineAligned) {
                    if (!isXspaceFor(m_lastWidth[m_area])) {
                        log.log(Level.FINEST, "Not enough X space for {0} - remaining {1} - Area={2}", new Object[]{m_lastWidth[m_area], getXspace(), m_area});
                        newLine();
                    }
    ...
    private PrintElement layoutTable(MPrintFormat format, PrintData printData,
        ...
        for (int c = 0; c < format.getItemCount(); c++) {
            ...
            if (format.getItem(c).isTRANSPORTED()) {
                transport.put(format.getItem(c).getColumnName(), columnCount - 1);
            }
        }
        ...
        for (int row = 0; row < rows; row++) {
            ...
            if (printData.isFunctionRow()) {
                ...
                if (printData.isPageBreak()) {
                    ...
                    log.log(Level.FINER, "PageBreak row={0}", row);
                }
            ...
            for (int c = 0; c < format.getItemCount(); c++) {
                MPrintFormatItem item = format.getItem(c);
                /*
                 * if ( item.isTransport() ){ transport.put(item.getColumnName(), columna); }else{ if
                 * (item.getColumnName() != null){ columna++; } }
                 */
        ...
        TableElement table = new TableElement(columnHeader,
                                              ...
                                              tf, pageBreak);
        table.setTransport(transport, columnHeader.length);
        table.layout(0, 0, false, MPrintFormatItem.FIELDALIGNMENTTYPE_LeadingLeft);
        verificarTransport(table);
     ...
     *
     * @return null to obtain all attribute values from the job's attribute set.
     */
