 ...
 * @author pablo
 */
public class UnvoidPayments {
    
        private static CLogger log = CLogger.getCLogger(UnvoidPayments.class);
    ...
    public static void main(String[] args) {
        ...
        log.info("Unovoid Payments   $Revision: 1.0 $");
        log.info("----------------------------------");
           
        Trx trx = Trx.get(Trx.createTrxName("UnvoidPayments"), true);
        ...
        try {
            ...
            while (rs.next()) {
                MPayment pay = new MPayment(ctx, rs, trx.getTrxName());
                if (!pay.getDocStatus().equals(DocAction.ACTION_Void)){
                    countS++;
                    continue;
                }
                    
                log.info("Processing payment: "+pay.getDocumentNo());
                if (pay.unVoidIt()){
                    pay.save();
                    count++;
                }
                    ...
                    countE++;
                     log.severe("Error procesing payment - " + pay + " - "+pay.getProcessMsg()); 
                }
            }
            rs.close();
            pstmt.close();
