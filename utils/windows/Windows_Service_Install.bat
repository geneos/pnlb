...
%COMPIERE_HOME%\utils\windows\JavaService.exe -install Compiere %JAVA_HOME%\jre\bin\server\jvm.dll -Xmx256M -Djava.class.path=%JAVA_HOME%\lib\tools.jar;%COMPIERE_HOME%\jboss\bin\run.jar -server %COMPIERE_JAVA_OPTIONS% -Djetty.port=%COMPIERE_WEB_PORT% -Djetty.ssl=%COMPIERE_SSL_PORT% -Djetty.keystore=%COMPIERE_KEYSTORE% -Djetty.password=%COMPIERE_KEYSTORE_PASSWORD% -start org.jboss.Main -params -c compiere -stop org.jboss.Main -method systemExit -out %COMPIERE_HOME%\jboss\bin\out.txt -current %COMPIERE_HOME%\jboss\bin
